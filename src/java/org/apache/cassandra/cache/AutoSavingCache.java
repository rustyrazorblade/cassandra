...
public class AutoSavingCache<K extends CacheKey, V> extends InstrumentingCache<K, V>
{
        ...
        if (dataPath.exists() && crcPath.exists())
        {
            ...
            try
            {
                ...
                while (in.available() > 0)
                {
                    ...
                    TableStore cfs = Schema.instance.getColumnFamilyStoreInstance(tableId);
        ...
        public void saveCache()
        {
            ...
            try (WrappedDataOutputStreamPlus writer = new WrappedDataOutputStreamPlus(streamFactory.getOutputStream(cacheFilePaths.left, cacheFilePaths.right)))
            {
                ...
                while (keyIterator.hasNext())
                {
                    ...
                    TableStore cfs = Schema.instance.getColumnFamilyStoreInstance(key.tableId);
                    if (cfs == null)
    ...
    {
        void serialize(K key, DataOutputPlus out, TableStore cfs) throws IOException;

        Future<Pair<K, V>> deserialize(DataInputPlus in, TableStore cfs) throws IOException;
    }
}
