...
public class ActiveRepairService implements IEndpointStateChangeSubscriber, IFailureDetectionEventListener, ActiveRepairServiceMBean
{
    ...
    public UUID prepareForRepair(UUID parentRepairSession, InetAddressAndPort coordinator, Set<InetAddressAndPort> endpoints, RepairOption options, boolean isForcedRepair, List<TableStore> tableStores)
    {
        long repairedAt = getRepairedAt(options, isForcedRepair);
        registerParentRepairSession(parentRepairSession, coordinator, tableStores, options.getRanges(), options.isIncremental(), repairedAt, options.isGlobal(), options.getPreviewKind());
        ...
        List<TableId> tableIds = new ArrayList<>(tableStores.size());
        for (TableStore cfs : tableStores)
    ...
    public synchronized void registerParentRepairSession(UUID parentRepairSession, InetAddressAndPort coordinator, List<TableStore> tableStores, Collection<Range<Token>> ranges, boolean isIncremental, long repairedAt, boolean isGlobal, PreviewKind previewKind)
    {
        ...
        {
            parentRepairSessions.put(parentRepairSession, new ParentRepairSession(coordinator, tableStores, ranges, isIncremental, repairedAt, isGlobal, previewKind));
        }
    }
    ...
    public synchronized ParentRepairSession removeParentRepairSession(UUID parentSessionId)
    {
        ...
        for (TableStore cfs : getParentRepairSession(parentSessionId).columnFamilyStores.values())
        {
    ...
    {
        private final Map<TableId, TableStore> columnFamilyStores = new HashMap<>();
        ...
        public ParentRepairSession(InetAddressAndPort coordinator, List<TableStore> tableStores, Collection<Range<Token>> ranges, boolean isIncremental, long repairedAt, boolean isGlobal, PreviewKind previewKind)
        {
        ...
        {
            return ImmutableSet.<TableStore>builder().addAll(columnFamilyStores.values()).build();
        }
