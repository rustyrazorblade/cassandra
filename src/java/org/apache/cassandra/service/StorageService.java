...
public class StorageService extends NotificationBroadcasterSupport implements IEndpointStateChangeSubscriber, StorageServiceMBean
{
    ...
    {
        StreamSupport.stream(TableStore.all().spliterator(), false)
    ...
    */
    ...
    public int forceKeyspaceCleanup(int jobs, String keyspaceName, String... tables) throws IOException, ExecutionException, InterruptedException
    {
        ...
        for (TableStore cfStore : getValidColumnFamilies(false, false, keyspaceName, tables))
        {
    ...
    {
        CompactionManager.AllSSTableOpStatus status = CompactionManager.AllSSTableOpStatus.SUCCESSFUL;
        for (TableStore cfStore : getValidColumnFamilies(true, false, keyspaceName, tables))
        {
    ...
    public int verify(boolean extendedVerify, boolean checkVersion, boolean diskFailurePolicy, boolean mutateRepairStatus, boolean checkOwnsTokens, boolean quick, String keyspaceName, String... tableNames) throws IOException, ExecutionException, InterruptedException
    {
        ...
        for (TableStore cfStore : getValidColumnFamilies(false, false, keyspaceName, tableNames))
        {
    ...
    {
        CompactionManager.AllSSTableOpStatus status = CompactionManager.AllSSTableOpStatus.SUCCESSFUL;
        for (TableStore cfStore : getValidColumnFamilies(true, true, keyspaceName, tableNames))
        {
    ...
    {
        for (TableStore cfStore : getValidColumnFamilies(true, false, keyspaceName, tableNames))
        {
    ...
    {
        CompactionManager.AllSSTableOpStatus status = CompactionManager.AllSSTableOpStatus.SUCCESSFUL;
        for (TableStore cfs : getValidColumnFamilies(false, false, keyspaceName, columnFamilies))
        {
            ...
            throws IOException
    {
        ...
        for (String table : tableList)
        {
            ...
            if (splittedString.length == 2)
            {
                ...
                TableStore tableStore = keyspace.getColumnFamilyStore(tableName);
                // As there can be multiple column family from same keyspace check if snapshot exist for that specific
                // columnfamily and not for whole keyspace

                if (tableStore.snapshotExists(tag))
                    throw new IOException("Snapshot " + tag + " already exists.");
    ...
    public Map<String, TabularData> getSnapshotDetails()
    {
        ...
        for (Keyspace keyspace : Keyspace.all())
        {
            ...
            for (TableStore cfStore : keyspace.getColumnFamilyStores())
            {
     ...
     */
    public Iterable<TableStore> getValidColumnFamilies(boolean allowIndexes, boolean autoAddIndexes, String keyspaceName, String... cfNames) throws IOException
    {
        Keyspace keyspace = getValidKeyspace(keyspaceName);
    ...
    */
    ...
    public List<Pair<Range<Token>, Long>> getSplits(String keyspaceName, String cfName, Range<Token> range, int keysPerSplit)
    {
        ...
        TableStore cfs = t.getColumnFamilyStore(cfName);
    ...
    private List<Pair<Range<Token>, Long>> getSplits(List<Token> tokens, int splitCount, TableStore cfs)
    {
    ...
    private List<DecoratedKey> keySamples(Iterable<TableStore> cfses, Range<Token> range)
    {
    ...
    protected synchronized void drain(boolean isFinalShutdown) throws IOException, InterruptedException, ExecutionException
    {
        ...
        try
        {
            ...
            TableStore.shutdownPostFlushExecutor();
    ...
    {
        TableStore.loadNewSSTables(ksName, cfName);
    }
    ...
    public void rebuildSecondaryIndex(String ksName, String cfName, String... idxNames)
    {
        ...
        TableStore.rebuildSecondaryIndex(ksName, cfName, indices);
    }
    ...
    {
        for (TableStore cfs : getValidColumnFamilies(true, true, ks, tables))
        {
            cfs.disableAutoCompaction();
        }
    }
