...
public class SinglePartitionReadCommand extends ReadCommand implements SinglePartitionReadQuery
{
    ...
    /**
     * Layer-1 gate — cheap, decided before any leg opens. Reuses the
     * same "repaired-status tracking off" test the other cursor gates use
     * ({@code controller.isTrackingRepairedStatus()}), plus two checks specific to this
     * path (no digest query — digest computation never reaches the serialization path the
    ...
    private ReadResponse queryStorageToResponseBytes(ReadExecutionController controller)
    {
        ...
        try
        {
            ...
            // Single-leg reads have no transcode path at all.
            ...
            {
                // Single-leg or no-surviving-sstable-leg reads have no transcode path at all;
                // decline, and the caller falls back to the default.
