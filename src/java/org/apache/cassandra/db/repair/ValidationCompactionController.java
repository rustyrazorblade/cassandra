 ...
 * {@link CassandraValidationIterator}) so the legacy and cursor-backed validation paths share one
 * construction site.
 * <p>
 * Constructed with {@code compacting = null} via the
 * {@link CompactionController#CompactionController(ColumnFamilyStore, long)} 2-arg constructor,
 * {@link CompactionController#shadowSources} always returns null/empty regardless of
 * {@code tombstoneOption}.  {@link #guaranteesNoShadowSources} exposes that invariant so
 * {@code CursorCompactor#isValidationSupported} can rely on it instead of checking
 * {@code tombstoneOption} or this concrete controller type.
 */
