...
/**
 * Cursor-backed counterpart to {@link CassandraValidationIterator}.  It shares the same lifecycle
 * (snapshot selection, {@link CassandraValidationIterator#getSSTablesToValidate}, memtable-range
 * flush, estimated-bytes/partitions bookkeeping), but reads sstables via partial-range
 ...
 * {@link org.apache.cassandra.db.compaction.CompactionIterator}.  It never writes output sstables
 * and never materializes real row/cell content, only a precomputed per-partition digest (see
 * {@link PrecomputedDigestPartition}), so the same {@link ValidationCompactionController} and
 * {@link CursorCompactor} scan-side gates apply as for {@link CassandraValidationIterator}.  It
 * overrides {@link #feed} to read the precomputed digest off {@link PrecomputedDigestPartition}
 * directly, instead of re-digesting {@link #next()}'s result; {@code ValidationManager#doValidation}
 * calls {@code feed} polymorphically and need not know this.
 * <p>
 * It checks support itself, in two stages, throwing {@link CursorValidationUnsupportedException} if
 * either rejects; {@link CassandraTableRepairManager} catches that to fall back to
 * {@link CassandraValidationIterator}.  First, {@link CursorCompactor#unsupportedMetadata} - cheap
 * and metadata-only - runs before any flush/snapshot I/O, so a permanently-unsupported schema does
 * not pay for a blocking flush before falling back.  Second,
 * {@link CursorCompactor#isValidationSupported} runs against the sstable set actually acquired
 * (live-selected or snapshot), not an approximation that could diverge from it.
 */
