...
public class Memtable implements Comparable<Memtable>
{
    ...
    private static MemtablePool createMemtableAllocatorPool()
    {
        ...
        switch (DatabaseDescriptor.getMemtableAllocationType())
        {
            case unslabbed_heap_buffers:
                return new HeapPool(heapLimit, DatabaseDescriptor.getMemtableCleanupThreshold(), new Table.FlushLargestColumnFamily());
            case heap_buffers:
                return new SlabPool(heapLimit, 0, DatabaseDescriptor.getMemtableCleanupThreshold(), new Table.FlushLargestColumnFamily());
            case offheap_buffers:
                if (!FileUtils.isCleanerAvailable)
                {
                    throw new IllegalStateException("Could not free direct byte buffer: offheap_buffers is not a safe memtable_allocation_type without this ability, please adjust your config. This feature is only guaranteed to work on an Oracle JVM. Refusing to start.");
                }
                return new SlabPool(heapLimit, offHeapLimit, DatabaseDescriptor.getMemtableCleanupThreshold(), new Table.FlushLargestColumnFamily());
            case offheap_objects:
                return new NativePool(heapLimit, offHeapLimit, DatabaseDescriptor.getMemtableCleanupThreshold(), new Table.FlushLargestColumnFamily());
            default:
    ...
    // has been finalised, and this is enforced in the Table.setCommitLogUpperBound
    ...
    public Memtable(AtomicReference<CommitLogPosition> commitLogLowerBound, Table cfs)
    {
        this.cfs = cfs;
    ...
    /**
     * Should only be called by Table.apply via Keyspace.apply, which supplies the appropriate
     * OpOrdering.
     *
    ...
    public static class MemtableUnfilteredPartitionIterator extends AbstractUnfilteredPartitionIterator
    {
        ...
        public MemtableUnfilteredPartitionIterator(Table cfs, Iterator<Map.Entry<PartitionPosition, AtomicBTreePartition>> iter, int minLocalDeletionTime, ColumnFilter columnFilter, DataRange dataRange)
        {
            this.cfs = cfs;
            this.iter = iter;
