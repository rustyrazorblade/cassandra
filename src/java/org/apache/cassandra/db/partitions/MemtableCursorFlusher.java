...
public class MemtableCursorFlusher
{
     ...
     * partition boundary whether to roll onto the next one. It mirrors
     * {@code CursorCompactor.OutputWriterProvider} on the flush side: the single-output case
     * ({@code SimpleSSTableMultiWriter}) always returns null from {@link #maybeSwitchWriter}, so the
     * flush stays on one writer exactly as before; the sharded case
     * ({@code ShardedMultiWriter} under {@code UnifiedCompactionStrategy}) returns that
     * multi-writer's own per-shard writers, so the cursor flush splits the output at the very same
     * shard boundaries the iterator path would - and into the very same writer instances the
     * multi-writer later finishes, commits, and applies token-space coverage to.
     */
    public interface OutputWriterProvider
    {
         ...
         * crosses a boundary after the current shard has data, otherwise null (the current writer
         * still owns {@code key}). Called once per partition, in token order.
         */
        SortedTableWriter<?, ?> maybeSwitchWriter(DecoratedKey key);
    }

    private final OutputWriterProvider writerProvider;
    // Re-pointed at each shard rollover (see maybeSwitchWriter); the cursor wrapping the shard
    // currently being written. Never null between construction and close.
    ...
    public static boolean isSupported(TableMetadata metadata, Memtable memtable)
    {
        ...
        if (CursorCompactor.unsupportedMetadata(metadata))
        {
            ...
            LOGGER.debug("Cursor flush is not supported for {}.{}: rejected by CursorCompactor.unsupportedMetadata (see above for the specific reason, if logged)",
                         metadata.keyspace, metadata.name);
            return false;
        }
    ...
    public void flush(Memtable.FlushablePartitionSet<?> toFlush) throws IOException
    {
        ...
        {
            // Close the current (final) shard's cursor - flushes its index-builder state into that
            // shard's SortedTableWriter. forFlush.close() leaves the underlying writer open: the
            // flush transaction drives its finish/close (per shard, via the multi-writer). Rolled-
            // away shards were already closed in maybeSwitchWriter, so only one cursor is ever open.
            writer.close();
        }
    }

    /**
     * On crossing a shard boundary, closes out the shard being left and opens the next. Mirrors
     * {@code CursorCompactor.maybeSwitchWriter}/{@code writerRollover}: stamp the leaving shard's
     * last key, then close its cursor (flushing the index builder) before opening the next.
     */
    private void maybeSwitchWriter(DecoratedKey key) throws IOException
    {
        ...
        // writePartition - it doubles as the "have we set this writer's first key yet" guard.
