 ...
 * {@link UnfilteredRowIterators#digest} would produce, without materializing the merged partition
 * as a real {@link org.apache.cassandra.db.rows.UnfilteredRowIterator}.  It feeds the digest once,
 * directly, during the merge.
 * <p>
 * <b>What is still built vs. avoided.</b> It still constructs the small, already-{@code digest()}-
 * implementing value objects: {@link Clustering}/{@link ClusteringBound}/{@link ClusteringBoundary}
 * (via {@link ClusteringDescriptor#toClusteringPrefix}), {@link DeletionTime}, {@link LivenessInfo},
 * {@link CellPath}, and a {@link BufferCell} per cell (reusing its correct
 * {@code isCounterCell()}/tombstone detection rather than re-deriving it from raw fields).  It avoids
 * {@link org.apache.cassandra.db.rows.Row.Builder}/{@link BTreeRow}/BTree construction per row,
 * {@link org.apache.cassandra.db.rows.ComplexColumnData} objects, the whole-partition
 * {@code List<Unfiltered>} plus its iterator wrapper, and the re-walk
 * {@code UnfilteredRowIterators.digest()} would perform over that structure.
 * <p>
 * <b>Field order.</b> Must match {@link UnfilteredRowIterators#digest} exactly: partition key,
 * partition deletion, regular column names, static column names (only if a static row is present -
 * the {@code CASSANDRA-12090} reference-equality check against {@code Rows.EMPTY_STATIC_ROW}), the
 * (always-{@code false}) reverse-order flag, the static row's digest, then each {@link Unfiltered}
 * in order.  A row's digest ({@link org.apache.cassandra.db.rows.AbstractRow#digest}) is kind byte,
 * clustering, row deletion, primary-key liveness, then each column.  The row deletion is
 * {@link org.apache.cassandra.db.rows.Row.Deletion#digest} - {@code DeletionTime.digest()} plus an
 * {@code isShadowable} boolean (always {@code false} here, since MV data is excluded from cursor
 * validation) - inlined rather than allocating a {@code Row.Deletion}.  A marker's digest
 * ({@link RangeTombstoneBoundMarker#digest}/{@link RangeTombstoneBoundaryMarker#digest}) has no
 * leading kind byte: bound, then deletion(s) only.
 * <p>
 * <b>Header timing.</b> A row's clustering is only known at {@code writeRowEnd}, and whether the
 * static row was present (vs. the {@code Rows.EMPTY_STATIC_ROW} sentinel) only once the static row
 * is resolved.  So the partition header (key, deletion, column names, static-columns-if-present,
 * reverse-order flag) AND the static row's digest are flushed together, once, in
 * {@link #ensureHeaderFlushed}: with the real static row's fields if one was written, otherwise with
 * the sentinel's fixed fields.  The static row's digest runs unconditionally either way, matching
 * {@code iterator.staticRow().digest(digest)}.  {@code ensureHeaderFlushed} is called from
 * {@link #writeEmptyStaticRow}, from the static row's {@code writeRowEnd}, and (idempotently) from
 * the first non-static row/marker or from {@code writePartitionEnd}.
 * <p>
...
public class DigestingCursorMergeSink implements CursorMergeSink
{
    ...
    private final AbstractType<?>[] clusteringTypesList;
    ...
    public DigestingCursorMergeSink(TableMetadata metadata)
    {
        ...
        this.clusteringTypesList = metadata.comparator.subtypes();
    ...
    @Override
    public void writePartitionEnd(byte[] partitionKey, int partitionKeyLength, DeletionTime partitionDeletionTime, int headerLength, ClusteringDescriptor lastName) throws IOException
    {
        ensureHeaderFlushed();
