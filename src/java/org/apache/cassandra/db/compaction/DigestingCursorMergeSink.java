 ...
 * by an {@code isShadowable} boolean, always {@code false} for cursor-merged output (there is no
 * shadowable-deletion concept in the {@link CursorMergeSink} contract) - inlined directly rather
 * than allocating a {@code Row.Deletion} per row. A marker's digest
...
public class DigestingCursorMergeSink implements CursorMergeSink
{
    ...
    private void ensureHeaderFlushed()
    {
        ...
        if (hasStaticRow)
            digestRow(Clustering.STATIC_CLUSTERING, staticLiveness, staticDeletion, staticColumnOps);
        else
            digestRow(Clustering.STATIC_CLUSTERING, LivenessInfo.EMPTY, DeletionTime.LIVE, List.of());
    }

    /**
     * Digests one {@code Unfiltered.Kind.ROW} - matches {@code AbstractRow.digest()}: kind byte,
     * clustering, row deletion (deletion time + the always-{@code false} isShadowable bit, matching
     * {@code Row.Deletion.digest()} - see class doc), primary-key liveness, then each column.
     */
    private void digestRow(Clustering<?> clustering, LivenessInfo liveness, DeletionTime rowDeletion, List<ColumnDigestOp> columnOps)
    {
        digest.updateWithByte(Unfiltered.Kind.ROW.ordinal());
        clustering.digest(digest);
        rowDeletion.digest(digest);
        digest.updateWithBoolean(false); // Row.Deletion.isShadowable, always false for cursor-merged output
    ...
    @Override
    public void writePartitionEnd(DecoratedKey decoratedKey, byte[] partitionKey, int partitionKeyLength, DeletionTime partitionDeletionTime, int headerLength) throws IOException
    {
        ensureHeaderFlushed();
    ...
    public void writeCellHeader(int cellFlags, ReusableLivenessInfo cellLiveness, ColumnMetadata cellColumn) throws IOException
    {
        ...
        pendingCellLocalDeletionTime = cellLiveness.localExpirationTime();
        pendingCellPath = null;
        pendingCellValue = null;
        hasPendingCell = true;
    }
    ...
    public void writeRowEnd(UnfilteredDescriptor rHeader, boolean updateClusteringMetadata) throws IOException
    {
        ...
        else
        {
            ...
            digestRow(clustering, pendingRowLiveness, pendingRowDeletion, pendingColumnOps);
        }
        pendingColumnOps.clear();
    }
