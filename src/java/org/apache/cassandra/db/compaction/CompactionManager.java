...
public class CompactionManager implements CompactionManagerMBean
{
    ...
    @VisibleForTesting
    final Multiset<TableStore> compactingCF = ConcurrentHashMultiset.create();
     ...
     */
    public List<Future<?>> submitBackground(final TableStore cfs)
    {
    ...
    public boolean isCompacting(Iterable<TableStore> cfses)
    {
        for (TableStore cfs : cfses)
    ...
    class BackgroundCompactionCandidate implements Runnable
    {
        ...
        BackgroundCompactionCandidate(TableStore cfs)
        {
    ...
    private AllSSTableOpStatus parallelAllSSTableOperation(final TableStore cfs, final OneSSTableOperation operation, int jobs, OperationType operationType) throws ExecutionException, InterruptedException
    {
    ...
    public AllSSTableOpStatus performScrub(final TableStore cfs, final boolean skipCorrupted, final boolean checkData,
                                           int jobs)
    ...
    public AllSSTableOpStatus performVerify(TableStore cfs, Verifier.Options options) throws InterruptedException, ExecutionException
    {
        assert !cfs.isIndex();
        return parallelAllSSTableOperation(cfs, new OneSSTableOperation()
        {
    ...
    public AllSSTableOpStatus performSSTableRewrite(final TableStore cfs, final boolean excludeCurrentVersion, int jobs) throws InterruptedException, ExecutionException
    {
        return parallelAllSSTableOperation(cfs, new OneSSTableOperation()
        {
            @Override
    ...
    public AllSSTableOpStatus performCleanup(final TableStore cfStore, int jobs) throws InterruptedException, ExecutionException
    {
    ...
    public AllSSTableOpStatus performGarbageCollection(final TableStore cfStore, TombstoneOption tombstoneOption, int jobs) throws InterruptedException, ExecutionException
    {
        assert !cfStore.isIndex();
    ...
    public AllSSTableOpStatus relocateSSTables(final TableStore cfs, int jobs) throws ExecutionException, InterruptedException
    {
     ...
     */
    public ListenableFuture<?> submitPendingAntiCompaction(TableStore cfs, Collection<Range<Token>> ranges, Refs<SSTableReader> sstables, LifecycleTransaction txn, UUID sessionId)
    {
     ...
     * @throws IOException
     */
    public void performAnticompaction(TableStore cfs,
    ...
    public void performMaximal(final TableStore cfStore, boolean splitOutput)
    {
        FBUtilities.waitOnFutures(submitMaximal(cfStore, getDefaultGcBefore(cfStore, FBUtilities.nowInSeconds()), splitOutput));
    }

    public List<Future<?>> submitMaximal(final TableStore cfStore, final int gcBefore, boolean splitOutput)
    {
    ...
    public void forceCompactionForTokenRange(TableStore cfStore, Collection<Range<Token>> ranges)
    {
    ...
    private static Collection<SSTableReader> sstablesInBounds(TableStore cfs, Collection<Range<Token>> tokenRangeCollection)
    {
    ...
    public void forceUserDefinedCompaction(String dataFiles)
    {
        ...
        Multimap<TableStore, Descriptor> descriptors = ArrayListMultimap.create();
        ...
        for (String filename : filenames)
        {
            ...
            TableStore cfs = Keyspace.open(desc.ksname).getColumnFamilyStore(desc.cfname);
        ...
        for (TableStore cfs : descriptors.keySet())
    ...
    {
        String[] filenames = dataFiles.split(",");
        HashMap<TableStore, Descriptor> descriptors = Maps.newHashMap();
        ...
        for (Map.Entry<TableStore,Descriptor> entry : descriptors.entrySet())
        {
            TableStore cfs = entry.getKey();
    ...
    public Future<?> submitUserDefined(final TableStore cfs, final Collection<Descriptor> dataFiles, final int gcBefore)
    {
        Runnable runnable = new WrappedRunnable()
        {
            protected void runMayThrow()
    ...
    private SSTableReader lookupSSTable(final TableStore cfs, Descriptor descriptor)
    {
     ...
     */
    public Future<?> submitValidation(final TableStore cfStore, final Validator validator)
    {
        Callable<Object> callable = new Callable<Object>()
        {
    ...
    public void disableAutoCompaction()
    {
        ...
        {
            for (TableStore cfs : Keyspace.open(ksname).getColumnFamilyStores())
                cfs.disableAutoCompaction();
        }
    }

    private void scrubOne(TableStore cfs, LifecycleTransaction modifier, boolean skipCorrupted, boolean checkData, boolean reinsertOverflowedTTL)
    {
    ...
    private void verifyOne(TableStore cfs, SSTableReader sstable, Verifier.Options options)
    {
     ...
     * @throws IOException
     */
    private void doCleanupOne(final TableStore cfs, LifecycleTransaction txn, CleanupStrategy cleanupStrategy, Collection<Range<Token>> ranges, boolean hasIndexes) throws IOException
    {
        assert !cfs.isIndex();
    ...
    private static abstract class CleanupStrategy
    {
        ...
        public static CleanupStrategy get(TableStore cfs, Collection<Range<Token>> ranges, int nowInSec)
        {
        ...
        {
            public Bounded(final TableStore cfs, Collection<Range<Token>> ranges, int nowInSec)
            {
        ...
        private static final class Full extends CleanupStrategy
        {
            ...
            public Full(TableStore cfs, Collection<Range<Token>> ranges, int nowInSec)
            {
                super(ranges, nowInSec);
                this.cfs = cfs;
            }
    ...
    public static SSTableWriter createWriter(TableStore cfs,
    ...
    public static SSTableWriter createWriterForAntiCompaction(TableStore cfs,
                                                              File compactionFileLocation,
     ...
     */
    @SuppressWarnings("resource")
    private void doValidationCompaction(TableStore cfs, Validator validator) throws IOException
    {
    ...
    private static MerkleTrees createMerkleTrees(Iterable<SSTableReader> sstables, Collection<Range<Token>> ranges, TableStore cfs)
    {
    ...
    @VisibleForTesting
    synchronized Refs<SSTableReader> getSSTablesToValidate(TableStore cfs, Validator validator)
    {
        ...
        try (TableStore.RefViewFragment sstableCandidates = cfs.selectAndReference(View.select(SSTableSet.CANONICAL, predicate)))
        {
     ...
     */
    private void doAntiCompaction(TableStore cfs, Collection<Range<Token>> ranges, LifecycleTransaction repaired, long repairedAt, UUID pendingRepair)
    {
    ...
    private int antiCompactGroup(TableStore cfs, Collection<Range<Token>> ranges,
    ...
    public static int getDefaultGcBefore(TableStore cfs, int nowInSec)
    {
    ...
    {
        public ValidationCompactionController(TableStore cfs, int gcBefore)
        {
            super(cfs, gcBefore);
        }
    ...
    public void interruptCompactionForCFs(Iterable<TableStore> cfss, boolean interruptValidation)
    {
    ...
    public void waitForCessation(Iterable<TableStore> cfss)
    {
        long start = System.nanoTime();
