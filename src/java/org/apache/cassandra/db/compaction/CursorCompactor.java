...
public class CursorCompactor extends CompactionInfo.Holder
{
     ...
     * as opposed to {@link #isSupported}, which gates the writing compaction path. Validation never
     * writes output and always reads partial ranges, so it differs in three ways: partial-range
     * scanners are expected, not rejected; the BIG/BTI output-format gate does not apply; and the
     * {@code tombstoneOption} gate is replaced by requiring
     * {@link AbstractCompactionController#guaranteesNoShadowSources} on the controller.  Checking the
     * capability rather than the concrete controller type keeps this class off {@code db.repair}.
     */
    public static boolean isValidationSupported(Collection<SSTableReader> sstables, AbstractCompactionController controller)
    {
        ...
        // rejects mid-read. Modern view maintenance no longer produces these (since CASSANDRA-13409),
        // so regular cursor compaction admits views. Validation stays conservative: a repair session
        // that hits one mid-merge fails the whole repair with a hard error, and repair can run
        // against sstables written by any version still in the supported upgrade path.
        if (metadata.isView())
        {
    ...
    // a no-op. Mirrors the legacy TopPartitionTracker.TombstoneCounter, applied to the MERGED stream
    // BEFORE the Purger runs: tombstones are counted post-shadow (a shadowed cell/deletion is
    // excluded, matching Row.Merger) but pre-purge (a gc-purgeable tombstone still counts).
    // Complex/collection deletions and static-row tombstones are excluded, matching TombstoneCounter.
    ...
    /**
     * Returns true if any input sstable's header has static columns.  The input headers, not
     * current metadata, decide this: after {@code ALTER TABLE ... DROP} of the last static column,
     * older sstables can still carry static rows.
     */
    ...
    private void mergeUnfiltereds(int partitionMergeLimit, DeletionTime mergedDeletion, DeletionTime activeDeletion) throws IOException
    {
        ...
        while (true)
        {
            ...
            else if (UnfilteredSerializer.isTombstoneMarker(flags))
            {
                ...
                activeDeletion = openRanges.active() == DeletionTime.LIVE ? mergedDeletion : openRanges.active();
            }
            else
            {
                                      ...
                                      boolean isRowDropped, boolean isStatic) throws IOException
    {
            ...
            if (liveness.isTombstone())
            {
                ...
                if (resolution == RIGHT)
                {
                    ...
                    if (source.state() == CELL_VALUE_START)
                    {
                        ...
                        tempCellValueLength1 = source.lastCellValueLength();
                    }
                }
                else if (resolution == COMPARE)
                {
                    ...
                    if (source.state() == CELL_VALUE_START)
                    {
                        ...
                        tempCellValueLength2 = source.lastCellValueLength();
                    }
        ...
        if (tombstoneLiveness != null)
        {
            ...
            if (Cell.Serializer.hasValue(cellFlags))
                ssTableCursorWriter.writeCellValue(tempCellBuffer1, tempCellValueLength1);
            return isRowDropped;
        }
    ...
    // Range-tombstone open-marker set, shared with CursorReadMerger.
    final OpenRangeDeletions openRanges = new OpenRangeDeletions();
    ...
    private boolean mergeRangeTombstones(int rangeTombstoneMergeLimit, DeletionTime partitionDeletion, boolean isFirstUnfiltered) throws IOException
    {
        ...
        if (openRanges.active() != DeletionTime.LIVE) {
            previousDeletionTimeInMerged = openRanges.copyOf(openRanges.active());
        }
        try
        {
            updateOpenMarkers(rangeTombstoneMergeLimit, partitionDeletion);

            DeletionTime newDeletionTimeInMerged = openRanges.active();
        ...
        finally
        {
            ...
            {
                openRanges.recycle((ReusableDeletionTime) previousDeletionTimeInMerged);
            }
        }
    }
    ...
    private void updateOpenMarkers(int rangeTombstoneMergeLimit, DeletionTime partitionDeletion)
    {
        for (int i = 0; i < rangeTombstoneMergeLimit; i++)
            openRanges.apply(sstableCursors[i].unfiltered(), partitionDeletion);
        openRanges.settle();
    }
    ...
    private void writerRollover()
    {
        if (ssTableCursorWriter != null) {
            ...
            // Stamp the output's last key only when a partition was actually written.
            // lastSource tracks partitions READ, not written, so it must not gate this:
            // the read-only validation path (mergeNextPartition) can read a partition and
            // then purge all of it past gc_grace, so nothing is written even though a
            // partition was read. In that case lastSource is non-null but
            // hasWrittenPartition() is false, and lastWrittenKey() would assert. Real
            // (writing) compaction always writes at least one partition, so it stamps.
            if (hasWrittenPartition())
    ...
    /**
     * Allocates a {@code size}-element cursor array, lets {@code populate} fill it, and closes any
     * cursors already opened if population throws, so a partial failure leaks no file handles.
     */
