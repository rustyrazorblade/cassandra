...
public class CompactionController implements AutoCloseable
{
    ...
    protected CompactionController(TableStore cfs, int maxValue)
    {
        this(cfs, null, maxValue);
    }

    public CompactionController(TableStore cfs, Set<SSTableReader> compacting, int gcBefore)
    {
        this(cfs, compacting, gcBefore, null,
             cfs.getCompactionStrategyManager().getCompactionParams().tombstoneOption());
    }

    public CompactionController(TableStore cfs, Set<SSTableReader> compacting, int gcBefore, RateLimiter limiter, TombstoneOption tombstoneOption)
    {
        assert cfs != null;
        this.cfs = cfs;
     ...
     * @return
     */
    public static Set<SSTableReader> getFullyExpiredSSTables(TableStore cfStore,
