...
public abstract class AbstractLocalProcessor implements Processor
{
    ...
    private CycleResult runCycle(Entry.Id entryId, Transformation transform, String transformStr, long commitStart, Retry retryPolicy)
    {
        ...
        // Deriving the proposal still happens under the lock: reject a transform that would bump the cluster common
        // serialization version while multi step operations are in progress, before it can reach the CAS.
        if (result.isSuccess())
        {
