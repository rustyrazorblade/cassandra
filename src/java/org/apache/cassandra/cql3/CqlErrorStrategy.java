...
public final class CqlErrorStrategy extends DefaultErrorStrategy
{
    ...
    /**
     * ANTLR 3 reported only the single token where the parse gave up.  ANTLR 4
     * reports the whole text span from the decision start to the offending
     * token, so "{... : ALL}" produces "at input 'ALL}'" instead of "at input
     * '}'".  Restore the ANTLR 3 message so it names just the offending token.
     */
    @Override
    protected void reportNoViableAlternative(Parser recognizer, NoViableAltException e)
    {
        String msg = "no viable alternative at input " + getTokenErrorDisplay(e.getOffendingToken());
        recognizer.notifyErrorListeners(e.getOffendingToken(), msg, e);
    }

    /**
     * ANTLR 3 rendered the end-of-input token as "EOF" in the expected-token
     * list of a mismatched-input error.  ANTLR 4 renders it as {@code <EOF>}.
     * Restore the ANTLR 3 spelling for the expected tokens.  The offending-token
     * display is left untouched, so an EOF offending token still reads
     * {@code <EOF>}.
     */
    @Override
    protected void reportInputMismatch(Parser recognizer, InputMismatchException e)
    {
        String expecting = e.getExpectedTokens().toString(recognizer.getVocabulary()).replace("<EOF>", "EOF");
        String msg = "mismatched input " + getTokenErrorDisplay(e.getOffendingToken()) + " expecting " + expecting;
        recognizer.notifyErrorListeners(e.getOffendingToken(), msg, e);
    }
}
