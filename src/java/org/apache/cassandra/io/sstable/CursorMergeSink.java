...
public interface CursorMergeSink
{
    ...
    void writePartitionEnd(DecoratedKey decoratedKey, byte[] partitionKey, int partitionKeyLength, DeletionTime partitionDeletionTime, int headerLength, ClusteringDescriptor lastName) throws IOException;

    boolean writeEmptyStaticRow() throws IOException;

    void writeRowStart(LivenessInfo livenessInfo, DeletionTime deletionTime, boolean isShadowable, boolean isStatic) throws IOException;
    ...
    void writeCellHeader(int cellFlags, CellLivenessInfo cellLiveness, ColumnMetadata cellColumn) throws IOException;

    int writeCellValue(SSTableCursorReader cursor, byte[] copyColumnValueBuffer) throws IOException;

    void writeCellValue(DataOutputBuffer tempCellBuffer, int rawValueLength) throws IOException;
