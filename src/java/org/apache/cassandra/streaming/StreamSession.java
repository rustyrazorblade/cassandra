...
public class StreamSession implements IEndpointStateChangeSubscriber
{
    ...
    {
        failIfFinished();
        Collection<Table> stores = getColumnFamilyStores(keyspace, columnFamilies);
        if (flushTables)
    ...
    private Collection<Table> getColumnFamilyStores(String keyspace, Collection<String> columnFamilies)
    {
        Collection<Table> stores = new HashSet<>();
    ...
    @VisibleForTesting
    public List<OutgoingStream> getOutgoingStreamsForRanges(Collection<Range<Token>> ranges, Collection<Table> stores, UUID pendingRepair, PreviewKind previewKind)
    {
        List<OutgoingStream> streams = new ArrayList<>();
        try
        {
            for (Table cfs: stores)
            {
     ...
     */
    private void flushSSTables(Iterable<Table> stores)
    {
