...
public interface StreamingFileSource extends Closeable
{
    ...
    class Buffered implements StreamingFileSource
    {
        ...
        {
            // The zero-copy path takes the channel through channel() and closes it itself. The encrypted
            // path reads through read() and never takes it, so close it here when it was never handed out.
            if (channel != null && !handedOut)
                channel.close();
        }
    }
