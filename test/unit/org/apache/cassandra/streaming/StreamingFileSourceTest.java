...
public class StreamingFileSourceTest
{
    ...
    @Test
    public void readingPastTheEndOfTheFileFails()
    {
        for (DiskAccessMode mode : new DiskAccessMode[]{ DiskAccessMode.standard, DiskAccessMode.direct })
        {
            try (StreamingFileSource source = StreamingFileSource.open(file, mode))
            {
                // ask for more bytes than the file has left, so the read runs off the end
                source.read(ByteBuffer.allocate(100), FILE_SIZE - 10);
                fail("reading past the end of the file must fail for " + mode);
            }
            catch (IOException e)
            {
                assertTrue("the failure must name the file for " + mode + ": " + e.getMessage(),
                           e.getMessage().contains(file.toString()));
                assertTrue("the failure must name the position for " + mode + ": " + e.getMessage(),
                           e.getMessage().startsWith("Unexpected end of") && e.getMessage().contains(" at "));
            }
        }
    }
