...
public class SSTableRewriterTest extends SSTableWriterTestBase
{
    ...
    public void testCanonicalView() throws Exception
    {
        ...
        )
        {
            ...
            while (ci.hasNext())
            {
                ...
                {
                    checked = true;
                    TableStore.ViewFragment viewFragment = cfs.select(View.selectFunction(SSTableSet.CANONICAL));
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE);
        final TableStore cfs = keyspace.getColumnFamilyStore(CF);
        truncate(cfs);
    ...
    public static SSTableReader writeFile(TableStore cfs, int count)
    {
        return Iterables.getFirst(writeFiles(cfs, 1, count * 5, count / 100, 1000), null);
    }

    public static Set<SSTableReader> writeFiles(TableStore cfs, int fileCount, int partitionCount, int cellCount, int cellSize)
    {
        int i = 0;
