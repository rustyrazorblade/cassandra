...
public class SSTableReaderTest
{
    ...
    private void clearAndLoad(Table cfs)
    {
        cfs.clearUnsafe();
        cfs.loadNewSSTables();
    }
    ...
    public void testPersistentStatisticsWithSecondaryIndex()
    {
        ...
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        Table store = keyspace.getColumnFamilyStore(CF_INDEXED);
        partitioner = store.getPartitioner();
    ...
    public void testOpeningSSTable() throws Exception
    {
        ...
        Table store = keyspace.getColumnFamilyStore(cf);
        store.clearUnsafe();
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        Table store = keyspace.getColumnFamilyStore("Indexed1");
        ...
        for(Table indexCfs : store.indexManager.getAllIndexColumnFamilyStores())
        {
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        final Table store = keyspace.getColumnFamilyStore("StandardLowIndexInterval"); // index interval of 8, no key caching
        CompactionManager.instance.disableAutoCompaction();
    ...
    private void assertIndexQueryWorks(Table indexedCFS)
    {
        assert "Indexed1".equals(indexedCFS.name);

        // make sure all sstables including 2ary indexes load from disk
        for (Table cfs : indexedCFS.concatWithIndexes())
            clearAndLoad(cfs);
