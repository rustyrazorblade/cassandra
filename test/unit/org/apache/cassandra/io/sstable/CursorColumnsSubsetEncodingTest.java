 ...
 *    superset sizes (finding #12 bug 2: a flipped mode selection made the deserializer
 *    read the wrong mode — silent corruption);
 *  - subsets whose present columns sort AFTER the last missing column (finding #12 bug 1:
 *    a wrong tail-loop bound silently dropped them from the encoding).
 *
...
public class CursorColumnsSubsetEncodingTest
{
    ...
    public void sweepSizesShapesAndBoundaries() throws Exception
    {
        ...
        for (int size : sizes)
        {
            ...
            {
                // structured shapes: leading block, trailing block (present tail after the
                // last missing index — the finding-#12 dropped-tail shape), even spread
