...
import org.apache.cassandra.streaming.StreamingDataOutputPlus.RateLimiter;
...
public class AsyncStreamingOutputPlusTest
{
    ...
    /**
     * When the guarded body of writeToChannel(ByteBuffer, RateLimiter) throws, the buffer the caller handed
     * over must still find its way back to the networking pool rather than leaking.
     */
    @Test
    public void writeToChannelReturnsTheBufferWhenTheGuardedBodyThrows() throws IOException
    {
        long used = BufferPools.forNetworking().usedSizeInBytes();

        RateLimiter throwing = new RateLimiter()
        {
            public void acquire(int bytes)
            {
                throw new RuntimeException("no permits");
            }
        ...
        {
            ByteBuffer buffer = BufferPools.forNetworking().get(16, BufferType.OFF_HEAP);
            buffer.putLong(1).putLong(2).flip();

            try
            {
                out.writeToChannel(buffer, throwing);
                fail("acquire threw, so the write must fail");
            }
            catch (RuntimeException e)
            {
                assertEquals("no permits", e.getMessage());
            }
        }

        assertEquals("the buffer must go back to the pool when the guarded write fails",
