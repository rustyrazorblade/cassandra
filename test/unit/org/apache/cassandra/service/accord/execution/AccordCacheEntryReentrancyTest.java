 ...
 * future callback might plausibly perform, and checks the outcome against L3 of
 * {@code formalise/accord/execution/INVARIANTS.md}: a notification handler must not mutate the queue of the entry
 * notifying it, as several mutators publish their state only after notifying. Nothing enforces that at runtime; it
 * holds because no production handler does it.
 *
 * <ul>
 *   <li>{@link Where#SAME_ENTRY}: the injected mutation targets the notifying entry, which L3 forbids. Enumerated and
 *       counted so that a shape change which stops producing it is noticed, but deliberately not performed - see
 *       {@code isForbiddenByL3}, the one place to change if a runtime guard is added.</li>
 *   <li>{@link Where#OTHER_ENTRY}: the injected mutation targets a different loaded entry that every task also holds a
 *       position on. L3 permits this and it is the production shape (a txnId promotion queues on keys from inside a
 *       notification via {@code waitOnKeysExclusive}), so this is where the coverage lives.</li>
 * </ul>
 *
 * <p>Enumerated exhaustively over {shape} x {driver} x {injection site} x {injected action} x {where} x {target}; a
 * configuration that cannot arise is counted as skipped, and the reachable total is asserted to be a substantial
 * fraction of the enumeration.
 *
