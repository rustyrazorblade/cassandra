...
public class SecondaryIndexTest
{
    ...
    {
        Table cfs = Keyspace.open(KEYSPACE1).getColumnFamilyStore(WITH_COMPOSITE_INDEX);
    ...
    public void testCompositeIndexUpdate() throws IOException
    {
        ...
        Table cfs = keyspace.getColumnFamilyStore(WITH_COMPOSITE_INDEX);
        ByteBuffer col = ByteBufferUtil.bytes("birthdate");
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        Table cfs = keyspace.getColumnFamilyStore(WITH_KEYS_INDEX);
    ...
    {
        Table cfs = Keyspace.open(KEYSPACE1).getColumnFamilyStore(WITH_KEYS_INDEX);
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        Table cfs = keyspace.getColumnFamilyStore(COMPOSITE_INDEX_TO_BE_ADDED);
        ...
        Table indexCfs = cfs.indexManager.getIndex(indexDef)
                                         .getBackingTable()
    ...
    private void assertIndexedNone(Table cfs, ByteBuffer col, Object val)
    {
        assertIndexedCount(cfs, col, val, 0);
    }
    private void assertIndexedOne(Table cfs, ByteBuffer col, Object val)
    {
        assertIndexedCount(cfs, col, val, 1);
    }
    private void assertIndexedCount(Table cfs, ByteBuffer col, Object val, int count)
    {
    ...
    private void assertIndexCfsIsEmpty(Table indexCfs)
    {
