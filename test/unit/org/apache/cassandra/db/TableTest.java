...
public class TableTest
{
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        final Table cfs = keyspace.getColumnFamilyStore(CF_STANDARD2);
    ...
    public void testClearEphemeralSnapshots() throws Throwable
    {
        ...
        Table cfs = Keyspace.open(KEYSPACE1).getColumnFamilyStore(CF_INDEX1);
        ...
        Table.clearEphemeralSnapshots(cfs.getDirectories());
...
//        Table cfs = Keyspace.open(ks).getColumnFamilyStore(cf);
...
//    }
    ...
    public void reTest(Table cfs, Runnable verify) throws Exception
    {
        verify.run();
        cfs.forceBlockingFlush();
        verify.run();
    ...
    private void assertRangeCount(Table cfs, ByteBuffer col, ByteBuffer val, int count)
    {
        assertRangeCount(cfs, cfs.metadata().getColumn(col), val, count);
    }

    private void assertRangeCount(Table cfs, ColumnMetadata col, ByteBuffer val, int count)
    {
    ...
    public void testScrubDataDirectories() throws Throwable
    {
        ...
        Table.scrubDataDirectories(cfs.metadata());
