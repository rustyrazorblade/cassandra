...
/**
 * Checks {@link RangeTombstoneListCursor} against {@link RowAndDeletionMergeIterator}'s
 * range-tombstone-marker output, across the configurations they can differ on: disjoint ranges,
 * adjacent ranges with equal deletion, adjacent ranges with differing deletion under every
 * inclusive/exclusive combination, ranges shadowed by the partition-level deletion, unbounded
 * ranges, and a randomized sweep.
 */
public class RangeTombstoneListCursorDifferentialTest
{
    ...
    private static List<MarkerEvent> viaCursor(RangeTombstoneList list, DeletionTime partitionDeletion, boolean removeShadowedData)
    {
        ...
        while (true)
        {
            // peekPosition(), hasOpen(), and openDeletion() must agree with moveNext().  A SKIPPED
            // result means try again, the same retry a row-interleaving caller does.
    ...
    public void adjacentSameDeletionMergesSilently()
    {
        ...
        // [1,5] and [5,10] touch at 5 with the same deletion.  Build them via addAll from two
        // separate lists so they stay separate entries rather than coalescing on insert.
    ...
    {
        // inclusive close and exclusive open touching at 5, so an INCL_END_EXCL_START_BOUNDARY.
        RangeTombstoneList list = new RangeTombstoneList(cmp, 2);
        add(list, bound(true, true, 1), bound(false, true, 5), 10, 0);
    ...
    /**
     * [1,5] and [5,10], both closed at 5, overlap at 5.  This is the shape that reaches
     * {@link RangeTombstoneListCursor#moveNext}'s deletion-time tie-break, tested in both priority
     * orders since the result flips with the order.
     */
    @Test
