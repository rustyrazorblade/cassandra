...
public class CounterCellTest
{
    ...
    private Cell createLegacyCounterCell(TableStore cfs, ByteBuffer colName, long count, long ts)
    {
    ...
    private Cell createCounterCell(TableStore cfs, ByteBuffer colName, CounterId id, long count, long ts)
    {
        ColumnMetadata cDef = cfs.metadata().getColumn(colName);
    ...
    private Cell createCounterCellFromContext(TableStore cfs, ByteBuffer colName, ContextState context, long ts)
    {
        ColumnMetadata cDef = cfs.metadata().getColumn(colName);
        return BufferCell.live(cDef, ts, context.context);
    }

    private Cell createDeleted(TableStore cfs, ByteBuffer colName, long ts, int localDeletionTime)
    {
        ColumnMetadata cDef = cfs.metadata().getColumn(colName);
