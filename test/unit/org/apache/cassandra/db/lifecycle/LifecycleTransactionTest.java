...
public class LifecycleTransactionTest extends AbstractTransactionalTest
{
    ...
    private static final class TxnTest extends TestableTransaction
    {
        ...
        private static Tracker tracker(Table cfs, List<SSTableReader> readers)
        {
        ...
        private TxnTest(Table cfs)
        {
            this(cfs, readers(0, 8, cfs));
        }

        private TxnTest(Table cfs, List<SSTableReader> readers)
        {
    ...
    private static SSTableReader[] readersArray(int lb, int ub, Table cfs)
    {
        return readers(lb, ub, cfs).toArray(new SSTableReader[0]);
    }

    private static List<SSTableReader> readers(int lb, int ub, Table cfs)
    {
