 ...
 * using SSTableRewriter, Table, LifecycleTransaction, TransactionLogs, etc
 */
public class RealTransactionsTest extends SchemaLoader
{
    ...
    public void testRewriteFinished() throws IOException
    {
        ...
        Table cfs = keyspace.getColumnFamilyStore(REWRITE_FINISHED_CF);
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE);
        Table cfs = keyspace.getColumnFamilyStore(REWRITE_ABORTED_CF);
    ...
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE);
        Table cfs = keyspace.getColumnFamilyStore(FLUSH_CF);
    ...
    private SSTableReader getSSTable(Table cfs, int numPartitions) throws IOException
    {
    ...
    private void createSSTable(Table cfs, int numPartitions) throws IOException
    {
    ...
    private SSTableReader replaceSSTable(Table cfs, LifecycleTransaction txn, boolean fail)
    {
