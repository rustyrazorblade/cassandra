...
public class CursorValidationTombstoneCountParityTest extends CQLTester
{
    ...
    // scenario that used to diverge.  Pre-purge counting must still match legacy here.
    ...
    {
        // A partition whose every cell is individually deleted (100 cell tombstones),
        // gc_grace_seconds=0, validated after grace.
    ...
    {
        // Cell tombstones, row deletions, range tombstones, a partition deletion, and a boundary
        // marker - all purgeable (gc_grace=0), validated far in the future.  Pre-purge counts must
        // still match the legacy TombstoneCounter across all shapes.
    ...
    {
        // Expired TTL cells (must NOT be counted pre-purge; legacy still sees them as expiring)
        // alongside real counter-column tombstones.  gc_grace large so nothing purges.
