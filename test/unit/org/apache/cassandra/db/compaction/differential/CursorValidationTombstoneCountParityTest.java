 ...
 * the count {@link CursorCompactor#mergeNextPartition} feeds a {@link TopPartitionTracker.Collector}
 * must equal, per partition, what the legacy {@link TopPartitionTracker.TombstoneCounter} produces
 * over the merged ({@link UnfilteredPartitionIterators#merge}) stream before the purger runs.  This
 * is pre-purge behaviour: a gc-purgeable tombstone is still counted, so the two paths agree even
 * when {@code gc_grace_seconds=0} would let the purger drop the tombstones.
 *
 * <p>Named anchor: asserts a second invariant beyond byte-identity (tombstone-count parity).  The
 * matrix covers digest identity; this pins count parity.  The full content space (tombstone shapes,
 * types, statics, counters) is sampled by the fuzz engine added in P2.
 */
public class CursorValidationTombstoneCountParityTest extends CQLTester
{
    ...
    /**
     * Named anchor: asserts tombstone-count parity across mixed tombstone shapes (cell, row,
     * range, partition, and boundary markers), all purgeable (gc_grace=0), validated far in the
     * future.  Pre-purge counts must match the legacy TombstoneCounter across all shapes.
     *
     * <p>Broader content coverage (all tombstone shapes, types, statics, counters, collections)
     * is driven by the fuzz engine (see ValidationDigestFuzzTest).
     */
    @Test
    public void tombstoneCountMatchesLegacy() throws Throwable
    {
        ...
        for (long pk = 0; pk < 12; pk++)
        {
            ...
            execute("DELETE FROM %s WHERE pk = ? AND ck >= 3 AND ck <= 10", pk);
        }
    ...
    /**
     * Legacy per-partition tombstone counts: the {@link TopPartitionTracker.TombstoneCounter}
     * applied to the merged, pre-purge stream, as {@code CompactionIterator} wires it.
     */
    ...
    /**
     * Widens the top-partition tracking thresholds (min = 1, cap effectively unbounded) so every
     * partition with at least one tombstone is retained for an exact comparison, restoring the
