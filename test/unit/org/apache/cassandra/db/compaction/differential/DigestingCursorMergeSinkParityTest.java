...
/**
 * Digest-parity coverage for {@link DigestingCursorMergeSink}.  Runs the same scenario matrix as
 * {@link ValidationDigestParityTest} (static row, complex column, counter, range tombstone) but
 * compares per-partition digest bytes, matching how {@code Validator.rowHash()} uses digests in
 * production: one fresh {@code Digest.forValidator()} per partition, not one hash across the table.
 */
public class DigestingCursorMergeSinkParityTest extends CQLTester
{
    ...
    // A nowInSec far enough ahead that gc_grace=0 tombstones and short TTLs are past their local
    // deletion/expiration times, exercising the purge and ttl-to-tombstone paths.
    private static long farFutureNowInSec()
    {
        return FBUtilities.nowInSeconds() + TimeUnit.DAYS.toSeconds(30);
    }

    // Purge parity: gc_grace_seconds=0 plus a far-future nowInSec makes every tombstone purgeable
    // (cell, row, range, partition, and fully-purged partitions), so the legacy drop and the
    // cursor skip must agree on partition count and digests.
    @Test
    ...
    // Two overlapping range tombstones with different timestamps merge to a boundary marker where
    // they meet, pinning the end-then-start deletion digest order.
    @Test
    ...
    // Row deletions plus the resurrect path: a newer cell survives an older row deletion.
    @Test
    ...
    // Simple (non-collection) column cell tombstones (DELETE of a specific regular column).
    @Test
    ...
    // Live TTL cells (expiring liveness digest) and TTL'd primary-key liveness.
    @Test
    ...
    // Cells expired at merge time convert to tombstones (validated at a far-future nowInSec so the
    // short TTL has expired, but gc_grace keeps them from being purged).
    @Test
    ...
    // Expiration straddling the boundary: validating just before a cell's expiration (still live)
    // and just after (expired to a tombstone) must both hold digest parity, and the two sides must
    // differ - proving the boundary is really crossed, not landed on the same side twice.
    @Test
    public void ttlExpirationStraddlingBoundary() throws Throwable
    {
        ...
        cfs.disableAutoCompaction();
        int ttl = 1000;
        // Lower bound on the cell write-second; the real localExpirationTime is at least
        // writeNowLowerBound + ttl, so the before/after values below keep a wide margin.
        ...
        // Confirm the boundary is straddled: a live expiring cell and a ttl-to-tombstone cell
        // digest differently, so the two runs must produce different per-partition digests.
    ...
    // Fixed-size primitives beyond bigint, including a fixed-size clustering column.
    @Test
    ...
    // Variable-size primitives beyond text, plus an empty (zero-length) blob value.
    @Test
    ...
    // Multi-component clustering (mixed types, mixed reversed order) with a range tombstone over
    // the clustering key.
    @Test
    ...
    // UDTs: frozen (single cell) and non-frozen (complex), plus a nested UDT.
    @Test
    ...
    // Tuples and frozen collections (single-cell blobs).
    @Test
    ...
    // Static complex columns, static-row-only partitions, and static counter columns.
    @Test
    ...
    /**
     * Same as {@link #assertPerPartitionDigestsMatch(ColumnFamilyStore)} but with an explicit shared
     * {@code nowInSec}; purge/expiry tests pass a far-future value so tombstones and expired TTL
     * cells become purgeable deterministically, not depending on wall-clock during the test.
     */
