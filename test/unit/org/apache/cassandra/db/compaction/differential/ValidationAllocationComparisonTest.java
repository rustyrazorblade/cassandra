...
/**
 * Measures total thread-allocated bytes for a full-table validation compaction, comparing the
 * legacy {@code CompactionIterator} path against the cursor-backed path
 * ({@link CursorCompactor#mergeNextPartition} + {@link DigestingCursorMergeSink}).
 * <p>
 * This is a measurement tool, not a correctness gate; {@code DigestingCursorMergeSinkParityTest} is
 * the dedicated digest-parity suite.  The per-iteration digest comparison here only guards against
 * a cheaper-but-wrong measurement.
 * <p>
 * The cursor path never materializes a {@link UnfilteredRowIterator} per partition; it feeds
 * {@code Digest.update(...)} directly from cursor primitives.  The reduction therefore covers both
 * the source-reading/merging side and the per-partition output the legacy path always builds.
 */
public class ValidationAllocationComparisonTest extends CQLTester
{
    ...
    public void compareLegacyAndCursorValidationAllocation() throws Throwable
    {
        ...
        // Each round rewrites the same primary keys, so every sstable carries a superseded version
        // of most of the data: the source-side redundancy the cursor path avoids materializing.
        ...
        for (int i = 0; i < WARMUP_ITERATIONS + MEASURED_ITERATIONS; i++)
        {
            ...
            // Check every iteration: both paths must scan the same partitions and produce identical
            // per-partition digests (one fresh digest per partition, as Validator.rowHash() does).
            // Otherwise a cheaper-but-wrong cursor path would look like a real allocation win.
                                                      ...
                                                      Collection<SSTableReader> sstables, long gcBefore) throws Exception
    {
        ...
        try (ValidationCompactionController controller = new ValidationCompactionController(cfs, gcBefore))
        {
            assertTrue("cursor validation must be supported for this scenario",
