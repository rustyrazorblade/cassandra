 ...
 *
 * Workload space (task-15 hardening): INSERT / UPDATE (no row liveness) / primary-key-only
 * INSERT (liveness, no cells) / INSERT USING TTL (long-lived, far from the expiry boundary) /
 * explicit USING TIMESTAMP collisions from a small pool (same-timestamp tie-breaks); null
 * values on non-key columns (cell tombstones on simple columns, complex deletions on
 * multi-cell columns; mapped to empty buffers on clustering columns); row deletes, partition
 * deletes, single-sided and prefix range deletes, multi-column cell deletes, static cell
 * deletes. Composite partition keys (1-2 components).
 *
...
public class RandomDifferentialCompactionTest extends DifferentialCompactionTester
{
    ...
    private void runOneExample(long seed) throws Throwable
    {
        ...
        // ~12% of non-key values are null: cell tombstones on simple columns, complex
        // deletions on multi-cell columns. The data generator maps null to an empty buffer
        // on clustering columns (null clustering is invalid; empty is legal and exercises
        // BTI byte-comparable empty components) and never applies the domain to partition keys.
        ...
        // overwritten primary keys exercise the tie-break rules (finding #4 class)
        ...
        for (int round = 0; round < rounds; round++)
        {
            ...
            // cell deletes: random subset of regular columns at a known row (complex deletion
            // when the column is multi-cell); occasionally a static cell delete instead
