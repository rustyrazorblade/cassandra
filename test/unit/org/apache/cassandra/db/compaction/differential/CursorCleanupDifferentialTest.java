...
public class CursorCleanupDifferentialTest extends CursorCleanupDifferentialTester
{
    ...
    /**
     * Preemptive open on the cleanup path, which never switches writers.
     * <p>
     * After the first partition every boundary takes the no-switch branch and calls
     * {@code maybeReopenEarly}. Legacy cleanup reaches the same call through
     * {@link org.apache.cassandra.io.sstable.SSTableRewriter#append}, so the two must agree. At the
     * default 50MiB interval the threshold never fires on test-sized data and the branch is inert;
     * an interval of 0 fires it at every boundary, so the reopen actually runs against a rewriter
     * that has had exactly one {@code switchWriter}.
     */
    @Test
