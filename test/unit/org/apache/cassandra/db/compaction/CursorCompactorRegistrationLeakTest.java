...
/**
 * Regression coverage for the validation {@link CursorCompactor} constructor: when it throws after
 * {@link ActiveCompactionsTracker#beginCompaction} has registered it, it must unregister from
 * {@link CompactionManager#active}.  Otherwise a failed setup leaves a phantom
 * {@code nodetool compactionstats} entry that never clears, because {@code finishCompaction}
 * otherwise only runs from {@code close()}, which a throwing constructor never reaches.
 */
public class CursorCompactorRegistrationLeakTest extends CQLTester
{
    ...
    public void constructorFailureAfterBeginCompactionUnregisters() throws Throwable
    {
        ...
        {
            // A range that ends before it starts makes the cursor constructor throw, from the
            // reader's first segment advance, which runs after beginCompaction has registered the
            // compaction.  An empty bounds list reads nothing, and overlapping ranges are only
            // caught once the reader advances to the second one, which construction never does;
            // neither reproduces the ctor-throws-after-register path.
            Map<SSTableReader, List<PartitionPositionBounds>> invertedBounds =
                Collections.singletonMap(sstable, Collections.singletonList(new PartitionPositionBounds(200, 100)));
            ...
            try
            {
                new CursorCompactor(OperationType.VALIDATION, invertedBounds, controller, nowInSec, compactionId,
