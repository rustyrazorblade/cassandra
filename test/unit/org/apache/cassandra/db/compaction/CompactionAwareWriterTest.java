...
public class CompactionAwareWriterTest extends CQLTester
{
    ...
    private Table getColumnFamilyStore()
    {
    ...
    public void testDefaultCompactionWriter() throws Throwable
    {
        ...
        Table cfs = ks.getColumnFamilyStore(TABLE);
    ...
    {
        Table cfs = getColumnFamilyStore();
        cfs.disableAutoCompaction();
        int rowCount = 1000;
        populate(rowCount);
        LifecycleTransaction txn = cfs.getTracker().tryModify(cfs.getLiveSSTables(), OperationType.COMPACTION);
    ...
    private int compact(Table cfs, LifecycleTransaction txn, CompactionAwareWriter writer)
    {
    ...
    private void validateData(Table cfs, int rowCount) throws Throwable
    {
