...
public class AntiCompactionTest
{
    ...
    private void antiCompactOne(long repairedAt, UUID pendingRepair) throws Exception
    {
        ...
        Table store = prepareColumnFamilyStore();
    ...
    private SSTableReader writeFile(Table cfs, int count)
    {
    ...
    public void generateSStable(Table store, String Suffix)
    {
        for (int i = 0; i < 10; i++)
        {
    ...
    private Table prepareColumnFamilyStore()
    {
        Keyspace keyspace = Keyspace.open(KEYSPACE1);
        Table store = keyspace.getColumnFamilyStore(CF);
        store.disableAutoCompaction();
    ...
    private static Set<SSTableReader> getUnrepairedSSTables(Table cfs)
    {
