...
/**
 * Verifies {@link CursorCounterContexts} produces byte-for-byte the same result as the upstream
 * {@link CounterContext} across randomized shard shapes: global, local, and remote mixes,
 * overlapping and disjoint CounterId sets, equal and differing clocks (including negatives),
 * superset relationships in both directions, marked-to-clear headers, and the legacy-shards
 * predicate.
 */
public class CursorCounterContextMergeTest
{
    ...
    public void mergeMatchesUpstreamAcrossRandomShapes()
    {
        ...
        // all three outcomes must actually occur
    ...
    public void clearMatchesUpstreamDeserializationBehavior()
    {
        ...
        for (int trial = 0; trial < 2000; trial++)
        {
            ...
            // half the trials run the streaming mark first
            if (random.nextBoolean())
                ctx = upstream.markLocalToBeCleared(ctx);

            // the iterator's clear-on-deserialize path
