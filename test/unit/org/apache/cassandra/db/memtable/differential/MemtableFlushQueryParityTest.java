...
{
    // The cursor flush path requires a heap-based memtable allocator; the ambient allocator
    // otherwise leaks in from config (ant test-latest selects offheap_objects). Pin it, so this
    // test owns its environment. The skiplist memtable is pinned inline on the DDL below.
