...
/**
 * Allocation gate for the cursor-served read slice (CASSANDRA-20428).
 *
 * Unlike {@code CursorCompactionAllocationGateTest}, which asserts an ABSOLUTE ceiling because
 * cursor compaction's property is garbage-freedom, the cursor read path claims NO allocation win
 * at this stage: it materializes the same {@code Row}/{@code Cell}/{@code BTreeRow} inventory as
 * the iterator path at the {@code UnfilteredRowIterator} boundary. The property to gate is
 * therefore COMPARATIVE parity: the same workload, run through both paths, must not allocate
 * meaningfully more on the cursor path than on the iterator path. The differential harness cannot
 * catch an allocation regression — the output bytes are identical whether or not the path
 * allocates — hence this separate gate (same rationale as the compaction gate).
 *
 * Method (mirrors the compaction gate): thread-allocated bytes via
 ...
 *
 * Workload: the read-path profile workload ({@code ReadPathAllocationProfileTest}) restricted to
 * gate-supported query shapes — single-partition, ASC, full-partition or single-slice,
 ...
 *
 * MARGIN ({@link #PARITY_MARGIN}), tightened against measured numbers:
 * <ul>
 ...
 * catches GROSS regressions (e.g. super-linear behavior). When the seek lands, that ceiling must
 * be tightened to the parity margin. The parity gate's slice shapes are near-full (excluding a
 * single row) so slice-handling code is exercised while the known eager materialization
 * divergence stays at one row per read.
 *
 * This test is the standing instrument for later work, which must show the cursor:iterator ratio
 * DECREASING below 1.0 as the merge and late materialization capture the real win.
 */
public class CursorReadAllocationGateTest extends CursorReadDifferentialTester
{
    ...
    /**
     * Loose tripwire for the known eager-materialization gap on mid-partition slices: each mid
     * slice covers 1/5 of the wide partition, so the cursor path materializes ~5x the iterator
     * path's row inventory by design; shared merge/orchestration/memtable costs pull the observed
     * ratio below that. 6.0 only catches gross regressions (e.g. materializing the partition once
     * per slice, or super-linear behavior). MUST be tightened to {@code 1 + PARITY_MARGIN} when
     * the BTI row-index seek lands.
     */
    ...
    public void cursorPathAllocationParityOnPhase1Slice() throws Throwable
    {
        ...
        assertTrue(String.format("cursor read path allocates more than the iterator path beyond the " +
                                 ...
                                 "the cursor read path claims allocation PARITY — a per-row/cell/leg " +
                                 "allocation has been introduced. Run " +
    ...
    public void widePartitionMidSliceEagerMaterializationStaysBounded() throws Throwable
    {
        ...
        double ratio = (double) cursorBest / iteratorBest;
        logger.info("cursor read mid-slice KNOWN-GAP tripwire (eager materialization, no seek): " +
                    "iterator={}B cursor={}B ratio={} ceiling={} — the seek must bring this to parity",
        ...
        assertTrue(String.format("mid-partition slice cursor allocation exceeds even the documented " +
                                 "eager-materialization gap: iterator=%,dB cursor=%,dB " +
    ...
    /** Mid-partition 1/5 slices of the wide partition: the known eager-materialization gap shape
     *  (cursor materializes all 1000 rows per leg, iterator index-seeks the slice). */
    ...
    /**
     * The read-path profile workload restricted to gate-supported shapes: {@value #FLUSH_ROUNDS}
     ...
     *  makes the gate measure nothing). Same discipline as the read-path profile harness. */
