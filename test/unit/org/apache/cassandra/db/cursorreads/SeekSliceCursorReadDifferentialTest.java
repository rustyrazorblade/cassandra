...
/**
 * Differential scenarios for bounded materialization: narrow slices of wide multi-index-block
 * partitions, slice bounds cutting range tombstones open at the seek point, partition-boundary
 * slices, and seeks past all rows.
 *
 * This class runs under the default (BIG) format, where the cursor path has no row-index seek: it
 * proves the scenario corpus is served correctly by the eager walk.
 ...
 *
 * The guard exists because the differential comparison alone cannot catch a broken seek: a cursor
 * path that materializes the whole partition and slices afterwards produces byte-identical results
 * by design. So on a seek-capable format every scenario additionally asserts, from
 * {@code CursorReads}' counters, that (a) the expected number of row-index seeks happened
 * ({@link CursorReads#sstableLegRowIndexSeeks}) and (b) the number of materialized unfiltereds
 * stayed bounded by index-block granularity instead of partition size
 * ({@link CursorReads#unfilteredsMaterialized}).
 *
...
public class SeekSliceCursorReadDifferentialTest extends CursorReadDifferentialTester
{
     ...
     * runs BIG; {@link BtiSeekSliceCursorReadDifferentialTest} overrides {@link #formatName()}.
     */
    @Before
    ...
    public void seekPastAllRows() throws Throwable
    {
        ...
        // the scenario would prove nothing about the seek. The sentinel keeps the slice inside the
    ...
    /**
     * A multi-leg read routes through the cursor-level merge core and each indexed BTI leg seeks
     * independently to its row-index floor block for the slice start ({@code seekForMerge}), so the
     * scenario asserts (a) the differential result stays byte-identical, (b) both legs actually
     * seeked (2 per run on a seek-capable format), (c) the merged materialization stayed
     * block-bounded instead of partition-sized, and (d) the merge core actually served the read.
     * {@code MergeSeekSliceCursorReadDifferentialTest} carries the dedicated merged-read corpus
     * (cross-leg open-RT seeding, mixed formats, the wrong-seed mutation test); this scenario keeps
     * this suite honest about merged reads.
     */
    @Test
