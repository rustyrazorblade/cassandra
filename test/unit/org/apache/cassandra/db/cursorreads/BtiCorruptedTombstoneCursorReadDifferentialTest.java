 ...
 * {@code UnfilteredValidation} wiring on cursor-served legs must behave identically — same
 * refusal, same diagnosis, same mark-suspect side effect — when the invalid deletions live in a
 * BTI sstable. The corruption fixtures are format-portable (both formats share the Data.db
 * unfiltered serialization, and BTI's only version, like the latest BIG version, uses the uint-ldt
 * encoding the far-future asymmetry pin depends on); what this subclass adds is the BTI lookup
