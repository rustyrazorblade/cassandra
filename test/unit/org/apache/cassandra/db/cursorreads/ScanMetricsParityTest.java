...
/**
 * Demonstrates and validates the {@link ScanMetricsCapture} harness — the metric-parity that a
 * filter-aware merge must satisfy (scan metrics are recorded below the row filter, so filter-dropped
 * rows still count; a merge that stops producing them must account for them identically).
 *
 * Two kinds of tests here:
 * <ul>
 *   <li><b>Parity</b> (iterator vs cursor path on identical commands): both paths feed the same
 *       {@code withMetricsRecording} stage with byte-identical streams. Covers the plain
 *       tombstone-heavy shape, LIMIT, a row filter (dropped rows still counted), and the warn /
 *       abort thresholds (exact-count parity via the warning text and abort message).</li>
 *   <li><b>Harness self-tests</b>: the capture must see what it claims to (known workload =>
 *       expected exact counts), and the parity assertion must fail on diverging observations
 *       (negative control).</li>
 * </ul>
 */
public class ScanMetricsParityTest extends CursorReadDifferentialTester
{
    ...
    public void parityAssertionDetectsDivergence() throws Throwable
    {
        ...
        // assertion must fail on it, or every parity test above proves nothing
    ...
    /**
     * The core parity check: capture the scan metrics of the same command through the iterator path
     * and the cursor path (with the standard silent-fallback guards) and assert every observation
     * identical. Returns the iterator-path snapshot for extra assertions.
     */
