...
/**
 * Response-serialization measurement and comparison plumbing for the transcode sink. A
 * replica-serving read serializes its merged, materialized result through
 * {@code ReadResponse.LocalDataResponse.build} — {@code serializerForIntraNode().serialize} into a
 * ByteBuffer. The transcode sink produces those same bytes without materializing; this class
 * provides both sides of every comparison the three verification layers need, so a producer only
 * has to plug a {@link ResponsePayloadProducer} in:
 * <ol>
 ...
 *       output — the byte-compare oracle the transcode sink's wire writer is verified against.</li>
 *   <li><b>Full payload</b> ({@link #intraNodePayload} vs a candidate producer): the exact
 *       response payload the production route emits, compared byte-for-byte.</li>
 ...
 *       the harness's canonical records — proving candidate bytes are readable by the real consumer
 *       and diagnosable record-by-record, not just equal.</li>
 * </ol>
...
public final class ResponseSerializationSupport
{
    ...
    /** The interface a transcode sink implements to join the comparisons: produce the intra-node
     *  response payload for a command by any means (materialize + serialize, or transcode from
     *  merge state). */
    @FunctionalInterface
    ...
    /** The unit oracle: each merged, materialized partition serialized independently via
