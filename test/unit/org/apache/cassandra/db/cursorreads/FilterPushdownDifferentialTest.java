...
/**
 * Differential scenarios for RowFilter pushdown's engagement gate
 ...
 *       the top filter keeps, which the byte comparison also catches).</li>
 ...
 *       dropped partition's rows are never materialized; byte-identity alone cannot see the
 *       difference). Includes the merged-static trap: the filtered static value is overwritten
 *       across legs, so a per-leg (non-merged) evaluation would compute the wrong verdict. Metrics
 *       parity on the drop case — with static-row tombstones present — is asserted via
 *       {@link ScanMetricsCapture}: the iterator path's filter closes the partition before its
 *       rows ever reach the (pull-based) {@code withMetricsRecording} stage, so neither path
 *       counts them, while the static row — pulled by the partition-level check — is counted by
 *       both.</li>
 *   <li><b>Mandatory fallback</b>: every unpushable shape — multi-cell CONTAINS, complex
 *       MAP_ELEMENT, counter column, needsReconciliation/non-strict filters, active
 *       query-size-tracking config, active purgeable-tombstone-recording config — must leave
 *       {@code filterPushdownEngaged} untouched while the query is still served (cursor legs
 *       advance; the counter column's filter is unpushable but the counter TABLE is now cursor-served
 *       under CASSANDRA-20428, so the filter simply stays above the merge). The
...
public class FilterPushdownDifferentialTest extends CursorReadDifferentialTester
{
    ...
    {
        Workload w = loadStaticWorkload(true);
        // partition-level PASSES, row-level (v1) drops most rows at production (regular-column
        // pushdown); the context attaches, the short-circuit must not fire, and byte identity
        // pins both verdicts
    ...
    public void staticFilterDropsPartitionWithMemtableLeg() throws Throwable
    {
        ...
        // short-circuit evaluates is assembled across sstable AND memtable legs (memtable adapter)
    ...
    /**
     * Metrics parity on a dropped partition that also carries static-row tombstones (the s2 static
     * cell is deleted in leg 1) and row tombstones (every 8th row): the static row is pulled by
     * the top filter's partition-level check on both paths (its dead cell reaches
    ...
    @Test
    public void counterColumnServedWithoutPushdown() throws Throwable
    {
        // Counter TABLES are now inside the cursor read gate (CASSANDRA-20428), so this read IS
        // cursor-served. A counter-column filter is still unpushable — filterPushdownFor's
        // defensive counter check (column.type.isCounter()) declines it — so the filter stays
        // above the merge and pushdown never engages, byte-identically to the iterator path.
        ...
        assertServedWithoutPushdown(cfs, cmd);
    }
    ...
    public void needsReconciliationFilterFallsBack() throws Throwable
    {
        ...
        // shape Util.cmd's filterOn builds, so those filtered differential scenarios remain
        // fallback scenarios by construction.
     ...
     *  {@code mergesStoppedByLimit} must stay untouched (no {@code DataLimits.Counter} bound
     *  exists for an unlimited query) — the composed engagement scenarios live in
     ...
     * When {@code flushFinal} is false the leg-1 writes stay in the MEMTABLE (the memtable-leg
     * merge shape); the caller then owns any further memtable writes.
     */
