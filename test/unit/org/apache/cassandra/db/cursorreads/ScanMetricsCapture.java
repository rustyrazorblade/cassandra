...
/**
 * Capture-and-compare harness for the scan metrics recorded by
 * {@code ReadCommand.withMetricsRecording} — the piece of the object-path stack that sits below the
 * row filter (MetricRecording), so filter-dropped rows still feed it. A filter-aware merge must keep
 * every one of these observations identical; this class lets differential scenarios assert metric
 * equality between two code paths, not just byte equality.
 *
 ...
 * </ul>
 *
 * Not captured (documented so callers don't assume otherwise):
