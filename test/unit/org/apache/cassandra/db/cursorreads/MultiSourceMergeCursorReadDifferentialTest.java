...
/**
 * Differential scenarios for the cursor-level merge across {@code >= 2} overlapping sstable legs
 * ({@code CursorReadMerger}): every scenario asserts byte-identical canonical records and intra-node
 * ReadResponse serialization against the iterator path (base harness), plus the merge-specific
 * effectiveness guards:
 * <ul>
 ...
 *       every correctness check (the result is still right) and fail exactly this.</li>
 * </ul>
 * Cross-source reconciliation edge cases covered: exact-timestamp cell ties (value tie-break
 * direction, the CASSANDRA-14592 expiring/tombstone-beats-live rules, lower-TTL preference),
 * partition-delete + reinsert split across legs, range tombstones opened in one leg and closed in
 * another (bound-to-boundary synthesis at merge), per-leg complex deletions vs cells, static-row
 * divergence, column-subset (canSkipValue) selection, and a {@code Slices.NONE}
 * partition-deletion-only inclusion leg. Also the merge core's own corruption mutation tests (wrong
 * merge decision, not just wrong materialization) and a per-leg validation scenario whose
 * corruption is shadowed out of the merged output (only per-leg validation can catch it —
...
public class MultiSourceMergeCursorReadDifferentialTest extends CursorReadDifferentialTester
{
     ...
     * harness executes the cursor path twice per call (canonical records + response bytes), hence
     * the doubling of every per-run expectation.
     */
    ...
    /**
     * On a 5-way shadow-heavy overlap, the merge must materialize only the merged winners —
     * asserted as unfilteredsMaterialized == merged output exactly (full-partition read: no
     * artificial slice markers, so the merged stream is the emitted stream), which is far below
     * Sigma(per-leg rows). Correctness alone cannot catch a fallback to per-leg materialization;
     * this does.
     */
    @Test
    ...
    /**
     * The merge-core mutation test: invert the shared resolveRegular verdict inside the merge so
     * reconciliation picks the losing cell, and prove the differential harness fails — i.e. it
     * catches a wrong merge decision, not just a wrong per-leg walk (which
     * {@code CursorReadGateTest.deliberateCorruptionIsDetected} already proves via the timestamp
     * hook, a hook that also applies inside the merge sink, covered here too).
     */
    @Test
                                        ...
                                        Runnable corrupt, Runnable restore, String label)
    {
        ...
        try
        {
            ...
            fail("differential harness FAILED TO DETECT " + label + " corruption of the merge core");
        }
