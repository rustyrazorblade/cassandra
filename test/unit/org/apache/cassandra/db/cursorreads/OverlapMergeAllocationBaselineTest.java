...
/**
 * Measures the per-source materialization cost that the cursor-level merge eliminates
 * (CASSANDRA-20428). When S sstables overlap on the same rows, each of the S legs is fully,
 * independently materialized by {@link CursorReads#sstableRowIterator} into real
 * {@code Row}/{@code Cell} objects before the object-level merge ({@code UnfilteredRowIterators
 * .merge}) discards the shadowed/superseded S-1 copies. A cursor-level merge reconciles at the
 * descriptor/byte level and materializes ONLY the merge winners, so the S-1 shadowed copies'
 * materialization is avoidable. Two numbers are captured per scenario:
 * <ol>
 *   <li><b>Total thread-allocated bytes per workload pass</b>, iterator path vs cursor path,
 *       via the {@code com.sun.management.ThreadMXBean} methodology
 ...
 *       {@link CursorReads#sstableRowIterator} per (command x live sstable), the same calls
 ...
 *       of the same commands, so the test-side accounting matches what production materializes.
 *       This fraction is the direct predictor of the merge's payoff — more precise than total
 *       allocation, which also carries per-leg constants and merge/orchestration costs common to
 *       both paths.</li>
 * </ol>
 *
 * Workload (S in {1, 2, 5}, shadow-heavy and tie-heavy variants; each scenario gets its own
 * table):
 * <ul>
 ...
 * partition per leg, so the measured delta is the merge shape itself, not the separate seek
 * story — the BTI mid-slice variant of {@link CursorReadAllocationGateTest} covers that).
 *
 * Multi-leg reads route through {@code CursorReadMerger}, so (a) the shadowed-waste cross-check
 * is "materialized == merged output" — the anti-fallback effectiveness guard — with exact
 * merge-served counter accounting, and (b) the S=5 scenarios carry the allocation-WIN gate
 * (cursor:iterator strictly below 1.0; bound documented at the assertion). The per-leg Sigma
 * enumeration below still measures the avoidable waste via direct single-leg
 * {@link CursorReads#sstableRowIterator} calls, which keep single-leg per-leg behavior by design.
 */
public class OverlapMergeAllocationBaselineTest extends CursorReadDifferentialTester
{
    ...
    private void runScenario(int sources, Variant variant) throws Throwable
    {
        ...
        logger.info("overlap-merge measurement [{}] (multi-leg reads are served by the cursor-level " +
                    "merge, single-leg by the per-leg path):\n" +
                    ...
                    "  DISCARDED by object merge:                          rows={} ({}) cells={} ({}) valueBytes={} ({})",
        ...
        // Allocation gate: with the cursor-level merge serving multi-leg reads, the cursor path
        // must allocate strictly LESS than the iterator path on the S=5 overlap workloads — the
        // per-leg shells/values of merge losers are no longer materialized. Bounds calibrated
        // empirically against measured steady-state ratios (JDK21):
                                      ...
                                      Counts legs) throws Throwable
    {
        ...
        try
        {
            ...
            // multi-leg reads must actually route through the cursor-level merge core (one merge
            // per read, every leg cursor-merged); single-leg reads must NOT
        ...
        // The counter measures the merge's effectiveness: for multi-leg reads it must equal the
        // MERGED output (winners only), far below Sigma(per-leg) on overlap workloads — a silent
        // fallback to per-leg materialization would still produce byte-identical results and fail
        // exactly this. Single-leg reads keep the per-leg path, where materialized == per-leg by
        // construction.
        if (sources == 1)
            assertEquals("single-leg pass must keep per-leg materialization accounting exactly",
