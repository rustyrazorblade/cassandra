 ...
 * {@code memtable_allocation_type: offheap_objects}, verifying the memtable adapter under the
 * off-heap memtable config too. Here the memtable's cells live off-heap ({@code NativeAllocator})
 * and {@code AtomicBTreePartition.unfilteredIterator} wraps them with
 * {@code EnsureOnHeap.CloneToHeap}; the adapter sits above that, inheriting its off-heap lifetime
 * safety, which is what makes the object reuse safe.
 *
 ...
 * initialization, and hard-assert the pool type per test so an already-initialized JVM cannot
 * silently run this suite under heap_buffers.
 */
