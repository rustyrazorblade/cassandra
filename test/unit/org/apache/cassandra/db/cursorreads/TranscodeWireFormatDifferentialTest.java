...
public class TranscodeWireFormatDifferentialTest extends CursorReadDifferentialTester
{
     ...
     *  optionally purged by a {@link PurgeFunction} subclass so gcable-tombstone scenarios compare
     *  against post-purge bytes, not the purge-free merge core's raw output.
     *  {@code purge == null} means no purging (the common case). */
    ...
    /** Full scenario: byte-compare (no purging) plus a round-trip through the production
     *  deserializer, plus the negative-control assertion that a corrupted transcode run is caught. */
    ...
    public void gcableTombstonesArePurgedAtEmission_onlyRepairedTombstones() throws Throwable
    {
        ...
        // tombstone here (localDeletionTime far below nowInSec) is treated as "repaired", so this
        // exercises the onlyPurgeRepairedTombstones branch, not just that purging happens.
    ...
    public void skewedTimestampHookIsCaughtByTheHarness() throws Throwable
    {
        ...
        {
            assertTranscodeMatchesMaterializedOracle(cfs, 1L);
            fail("expected the byte-comparison harness to catch the skewed timestamp");
        }
    ...
    public void wrongFlagsHookIsCaughtByTheHarness() throws Throwable
    {
        ...
        {
            assertTranscodeMatchesMaterializedOracle(cfs, 1L);
            fail("expected the byte-comparison harness to catch the wrong flags byte");
        }
        catch (AssertionError expected)
        {
            assertTrue("expected a BYTE divergence assertion, got: " + expected.getMessage(),
