...
/**
 * Allocation measurement of the LIMIT, RowFilter, paging and replica-response-serialization
 * shapes (CASSANDRA-20428). With the limit-driven production bound in place, the LIMIT and paging
 * materialization assertions are payoff gates: materialization on bounded shapes must scale with
 * rows RETURNED, and engagement is guarded through {@link CursorReads#mergesStoppedByLimit} in
 * both directions (bounded shapes trip it, unlimited/filtered shapes must not). The filter shapes
 * keep their eager-production assertions. Allocation ratios are logged, not asserted, as in
 * {@link OverlapMergeAllocationBaselineTest}.
 *
 * Measured per shape, over a {@value #WIDE_ROWS}-row partition merged from {@value #SOURCES}
 * fully-overlapping sstables:
 * <ul>
 *   <li><b>Allocation per pass</b>, iterator vs cursor path, via the
 *       {@code com.sun.management.ThreadMXBean} warmup + min-of-N methodology with exact
 *       production-counter accounting in both directions (silent-fallback guard). On LIMIT/paged
 *       shapes the cursor path allocates more than the iterator path today — the iterator path
 *       stops consuming at the limit while the cursor merge is eager — so this is recorded, not
 *       asserted against.</li>
 ...
 *       Asserted exactly equal to today's behavior so any change fails this test and forces the
 *       numbers to be re-recorded.</li>
 *   <li><b>Response serialization</b> ({@code LocalDataResponse.build}'s encoding via
 *       {@code serializerForIntraNode().serialize}) measured as its own pass shape.</li>
 *   <li><b>Memtable-hot overlap</b> (2 sstable legs + 1 memtable leg in the same cursor merge).</li>
 * </ul>
 */
public class LateMaterializationAllocationBaselineTest extends CursorReadDifferentialTester
{
    ...
    private SSTableFormat<?, ?> baseOriginalFormat;

    /** This baseline encodes the BIG-format numbers.  Pin BIG so the test is correct under any
     *  harness config, including test-latest (which sets bti).  The BTI subclass pins bti and
     *  records its own numbers; its @Before runs after this one and overrides the format. */
    @Before
    public void pinBigFormat()
    {
        baseOriginalFormat = DatabaseDescriptor.getSelectedSSTableFormat();
        DatabaseDescriptor.setSelectedSSTableFormat("big");
    }

    @After
    public void restoreBaseFormat()
    {
        DatabaseDescriptor.setSelectedSSTableFormat(baseOriginalFormat);
    }
    ...
    private static final class ShapeResult
    {
        ...
        /** Merges whose production the limit bound stopped, per pass — the engagement guard
    ...
    public void limitShapesOverWideMergedPartition() throws Throwable
    {
        ...
        // Payoff gate: the merge's production is bounded by the query's limit, so materialization
        // scales with rows RETURNED, not with the slice size. Exact equality — this workload has
        // no dead rows or markers, so the bound must stop after precisely `limit` merged rows.
        ...
        assertEquals("LIMIT 16 must materialize exactly the rows it returns",
                     16, limit16.materializedPerPass);
        assertEquals("LIMIT 1 must materialize exactly the row it returns",
    ...
    public void filterShapesOverWideMergedPartition() throws Throwable
    {
        ...
        {
            // wideCommand()'s filterOn builds a needsReconciliation RowFilter
            // (AbstractReadCommandBuilder.filter = RowFilter.create(true)), which filterPushdownFor
            // always declines. These three scenarios measure that fallback shape's eager cost; the
            // pushable+limited shape is measured separately by
            ...
            assertEquals("filters run above the merge today: materialization must not depend on " +
                         "the filter verdicts (this fallback shape is not pushable)",
                         WIDE_ROWS, r.materializedPerPass);
            // scoping guard: the limit bound must NOT engage when a RowFilter is present and is not
            // fully pushable — the filter drops rows below the counter, so a filter-blind bound
            // could under-produce
    ...
    {
        // A filtered+limited query now gets a production bound: before, it paid the same full
        // eager materialization cost as an unfiltered, unlimited read. This scenario builds a
        // genuinely STRICT, fully-pushable filter directly (not via wideCommand's
        // needsReconciliation-shaped filterOn) keeping the second half of the partition (v3 >=
        // 500): a LIMIT must walk past the first 500 filter-dropped rows to find its matches,
        // composing both bounds.
        ColumnFamilyStore cfs = loadWideWorkload(SOURCES, true);
        long now = FBUtilities.nowInSeconds();
        long keepFrom = 500;
        ...
        // Payoff gate: unfilteredsMaterialized() counts only rows that reach the output
        // (endRow()/addRow() on the base materializer) — a filter-dropped or filter-abandoned row
        // never does, regardless of how many rows the merge walked past to find its keepers (that
        // walk cost shows up in the allocation numbers logged above). So materialization must
        // equal exactly the rows the query returns, as in the pure-LIMIT gate above.
        ...
        assertEquals("filtered LIMIT 16 must materialize exactly the rows it returns",
                     16, filteredLimit16.materializedPerPass);
        assertEquals("filtered LIMIT 1 must materialize exactly the row it returns",
    ...
    {
        // 2 flushed rounds + 1 unflushed round: the memtable leg joins the cursor merge
    ...
    /**
     * A genuinely STRICT, reconciliation-free, fully-pushable filter ({@code v3 >= keepFrom} on
     * the regular int column that mirrors {@code ck}) combined with an optional LIMIT — built
