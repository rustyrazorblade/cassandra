...
/**
 * Self-tests for {@link ResponseSerializationSupport}, run over a merged multi-source workload with
 * tombstones, a range tombstone and collections so the serialized grammar exercises deletions,
 * markers, complex columns and header-delta encoding — not just plain rows. Using the identity
 * candidate (the materialize-then-serialize path), it establishes that every comparison layer the
 * transcode-sink verification needs is in place and non-trivial: byte-compare against the real
 * {@code LocalDataResponse.build} route, unit-oracle assembly, round-trip through the production
 * deserializer, and corruption detection in both the byte and record dimensions.
 */
public class ResponseSerializationSupportTest extends CursorReadDifferentialTester
{
    ...
    public void harnessPayloadIsTheProductionBuildOutput() throws Throwable
    {
        ...
        assertTrue("empty payloads would make every comparison prove nothing", viaHarness.length > 0);
    }
    ...
    {
        Workload w = load();
        // layer 1: per-partition serialize-the-materialized-output bytes, composed with the
    ...
    {
        Workload w = load();
        // layer 3: payload bytes must be readable by the real response consumer and decode
    ...
    public void byteCorruptionIsDetected() throws Throwable
    {
        ...
        fail("byte comparison accepted a corrupted payload — the oracle would prove nothing");
    }
    ...
    public void wrongContentIsDetectedByRoundTrip() throws Throwable
    {
        ...
        fail("record comparison accepted a wrong-content payload — the round-trip layer proves nothing");
    }
    ...
    {
        Workload w = load();
        // the side the cursor path will actually compare: same harness, cursor path enabled, with
        // the standard silent-fallback guards; payload must match the iterator path's byte-for-byte
        ...
        try
        {
            ...
            assertTrue("workload is not supported by the cursor read gate; this check would prove nothing",
