...
/**
 * Differential scenarios for the row-index seek integrated into the multi-leg cursor merge: narrow
 * mid-partition slices on merged wide partitions, slice bounds landing inside range tombstones
 * opened in one leg while the merge's cross-leg open-marker set reconciles them against other legs'
 * contributions (each seeked leg's open-RT state is seeded from its own row index's
 ...
 * (slice start side) with the merge-core end-stop (slice end side).
 *
 ...
 * seek-effectiveness guard active ({@link #seekCapableFormat()}). The guard exists because the
 * differential comparison alone cannot catch a merged path that silently falls back to the eager
 * walk (byte-identical output by design): on a seek-capable format every scenario additionally
 * asserts the exact per-leg seek count and a block-granular bound on merged materialization.
 *
 * The mutation test ({@link #deliberateSeekSeedCorruptionIsDetected}) proves the harness detects a
 * per-leg open-marker seed that is dropped or carries a wrong value — the failure mode that would
 * silently produce wrong merged range-tombstone output for the query class this feature targets.
 *
...
public class MergeSeekSliceCursorReadDifferentialTest extends CursorReadDifferentialTester
{
    ...
    /**
     * Pins the on-disk format the scenario controls, so the corpus runs the same way under any
     * ambient config (for example {@code ant test-latest}, whose overlay selects BTI). The base
     * runs BIG; {@link BtiMergeSeekSliceCursorReadDifferentialTest} overrides {@link #formatName()}.
     * The mixed BIG+BTI scenario below still sets the format per flush inside the test itself.
     */
    @Before
     ...
     * bytes) plus the merge guards (merge served, legs merged) plus, on a seek-capable format, the
     * seek-effectiveness guard. The harness executes the cursor path twice per call (canonical
     * records + response bytes), hence the doubling.
     *
    ...
    /**
     * A narrow mid-partition slice on a 2-leg merged wide partition, with both slice bounds set —
     * the seek bounds the walk's start (each leg jumps to its floor block instead of walking ~1000
     * pre-slice rows) and the merge-core end-stop bounds its end, compounding to a block-granular
     * merged walk.
     */
    @Test
    ...
    /**
     * The mutation test: prove the differential harness catches a wrong per-leg open-marker seed —
     * both a seed that is silently dropped at merge start (the seeded deletion's shadowing and the
     * artificial slice-start open marker vanish; rows it deleted resurrect) and a seed carrying a
     * skewed value (the merged artificial open marker's deletion time is wrong). The scenario is
     * built so the seeded tombstone's close sits beyond the slice end: the corruption then produces
     * silently-wrong output rather than an internal close-matching failure, which is the dangerous
     * case the harness must detect. Only meaningful where seeks happen, hence the format
     * assumption.
     */
    @Test
    public void deliberateSeekSeedCorruptionIsDetected() throws Throwable
    {
        ...
        // and the slice start. The block gap makes the seed matter: the floor block for the slice
        // start contains only resurrected rows, so the seeked stream never sees the open marker and
        // the row index's openDeletion is the only source of the open deletion.
        ...
        // and an in-stream marker would mask a dropped seed.)
        insertWideRows(1000);
