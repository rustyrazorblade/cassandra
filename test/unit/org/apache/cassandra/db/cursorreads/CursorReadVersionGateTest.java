 ...
 *
 * This tests the gate LOGIC against real {@link SSTableReader} instances whose descriptors carry
 * a non-latest {@link Version} ({@link MockSchema#sstableWithVersion}), not an end-to-end read
 * over genuine legacy-format fixtures. Each scenario first asserts the gate is OPEN for the same
 * command/table with a latest-version sstable, so a {@code false} on the non-latest list is
 * attributable to the version arm and not to some other arm rejecting the mock setup.
 */
