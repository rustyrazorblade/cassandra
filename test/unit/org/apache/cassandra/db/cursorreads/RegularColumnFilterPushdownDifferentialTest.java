...
/**
 * Differential and scan-metrics-parity scenarios for REGULAR-column (value-window) filter
 * pushdown. Rows whose regular-column expression fails at winner resolution inside the cursor
 * merge's cell walk are abandoned (never emitted), while every scan metric, threshold, warning
 * text and abort the iterator path's {@code MetricRecording} would have recorded is preserved
 * through the {@code ScanStatsAccumulator}.
 *
 * What these scenarios pin down, beyond the clustering pushdown coverage:
 * <ul>
...
public class RegularColumnFilterPushdownDifferentialTest extends CursorReadDifferentialTester
{
    ...
    public void memtableWonFilterCells() throws Throwable
    {
        ...
        // existingCell().buffer() directly (no staging, no windowing) — and the memtable cell
        // escape hatch must keep firing for kept rows
    ...
    public void memtableOnlyRowsThroughEscapeHatch() throws Throwable
    {
        ...
        // the whole-row escape hatch, so the regular-column verdict runs the real
    ...
    {
        // the filter column (z1) sorts LAST in the cell order, so the a1/a2 cells of every row
        // are merged — and for started rows, emitted into the row builder — before the z1 winner
        // resolves and fails. The abandoned rows' earlier cell work is bounded wasted allocation;
        // their accounting must still classify exactly like a pre-cell rejection, which the
    ...
    public void abortThresholdCrossedOnlyByRegularDroppedRowTombstones() throws Throwable
    {
        ...
        try
        {
            // under regular abandonment the crossing happens entirely inside abandoned rows, so
            // the cursor path must throw at merge time with the same message (count and
            // last-scanned clustering) and the same metric side effects
            Supplier<SinglePartitionReadCommand> cmd =
                w.strictFiltered(f -> f.add(w.col("v1"), Operator.EQ, ByteBufferUtil.bytes(20L)));
            ScanMetricsCapture.Snapshot iterator = assertScanMetricsParity(w.cfs, cmd, true);
    ...
    {
        Workload w = loadMixedWorkload();
        // a fully pushable filter under a CQL_LIMIT engages row-level pushdown too. v1 = 20
        // matches exactly one row in the whole 64-row partition (ck = 2) — far short of LIMIT 10,
        // so the top counter is never satisfied and the merge must scan the entire partition
        // (mergesStoppedByLimit must NOT advance, exactly like the iterator path's counter). Every
        // one of the ~63 non-matching rows is dropped/abandoned at production rather than riding
        // along to the top filter unfiltered. The scenario where the limit does get satisfied
        // lives in LimitedFilterPushdownDifferentialTest.
        Consumer<RowFilter> filter = f -> f.add(w.col("v1"), Operator.EQ, ByteBufferUtil.bytes(20L));
    ...
    {
        // SIMPLE kind on a COMPLEX column (multi-cell set CONTAINS needs a materialized
        // ComplexColumnData) disengages the whole pushdown — the query is still cursor-served,
        // filtered entirely up top
    ...
    {
        // negative control on the abandonment path: the skew hook silently skips the dropped
        // rows' tombstone accounting, and the parity harness must fail on it
        Workload w = loadMixedWorkload();
        Supplier<SinglePartitionReadCommand> cmd =
            w.strictFiltered(f -> f.add(w.col("v1"), Operator.EQ, ByteBufferUtil.bytes(20L)));
    ...
    protected final class Workload
    {
        ...
        /** The same strict filter under a CQL LIMIT — row-level pushdown engages when the filter
         *  is fully pushable, cooperating with the limit bound; see
         *  {@code limitedQueryEngagesRegularPushdownWhenFilterIsFullyPushable}. */
    ...
    /**
     * The mixed workload (known scan-metric inventory: {@value #TOTAL_LIVE} live rows,
     * {@value #TOTAL_TOMBSTONES} tombstones), filtered on the REGULAR column v1 instead of the
     * clustering key. The v1 landscape after the merge:
     * <ul>
     ...
     *                    joins the merge; overwritten rows' winning v1 cells are memtable-won)
     */
