...
/**
 * Differential scenarios for the query shapes that limit-bounded and filter-aware production will
 * change, run over the existing merge-everything-then-filter path. Every scenario must pass now —
 * the top-of-stack {@code limits().filter} / {@code rowFilter().filter} transformations are
 * untouched by the cursor path, so byte-identity is expected — and this corpus is the safety net
 * those increments extend: when a production bound below the merge starts stopping row production
 * early, or a pushed-down filter starts abandoning row groups, these exact scenarios must still
 * byte-match, page-match and (via {@link ScanMetricsParityTest}) metric-match.
 *
 ...
 *       {@code CQLCounter}; landing on a row whose cells merge from all three legs; landing on a
 *       live row inside an open range tombstone (the stream ends mid-open-RT and
 *       {@code RTBoundCloser} synthesizes the artificial close); and LIMIT over gcable tombstones
 *       interleaved with live rows (purged by {@code withoutPurgeableTombstones} between the merge
 *       and the counter).</li>
 *   <li>RowFilter shapes: {@code SimpleExpression} with a value-comparing operator on a clustering
 *       column and on a simple regular column, a filter whose column is shadowed by a range
 *       tombstone on some rows (liveness-at-nowInSec must gate the value comparison), and the
 *       fallback shapes that must never be pushed down: multi-cell CONTAINS (needs materialized
...
public class LateMaterializationDifferentialTest extends CursorReadDifferentialTester
{
    ...
    // ---------------------------------------------------------------- LIMIT scenarios
    // (each asserts byte-identity AND that the production bound actually engaged — the bound stops
    // production exactly where the top-level counter stops consuming, so a silently non-engaging
    // bound would be byte-identical and only the counter guard catches it)
    ...
    public void filterOnClusteringColumn() throws Throwable
    {
        ...
        // shape evaluated from the descriptor's clustering wire bytes
    ...
    public void filterOnCellShadowedByRangeTombstone() throws Throwable
    {
        ...
        // check is what drops it, exactly the semantics a pushed-down evaluation must match.
    ...
    public void pagingAcrossMergedPartition() throws Throwable
    {
        ...
        // row, both RT regions and the multi-leg row-group band. The bound tracks each page
        // command's LIVE-row counter, which stops at-or-after the pager's own EMITTED-row counter
        // — dead rows make the bound produce slightly more than the page returns, never less (the
        // conservative direction).
        long stopped = CursorReads.mergesStoppedByLimit();
        assertPagedReadMatchesIterator(w.cfs, w.base, 16, 8);
        assertTrue("production bound did not engage during the paged sequence",
    ...
    public void perPartitionLimitWithPagingResume() throws Throwable
    {
        ...
        // remaining in the partition — the paging-seeded counter shape the bound must track
        Supplier<SinglePartitionReadCommand> cmd = () -> {
            SinglePartitionReadCommand base = w.base();
            return SinglePartitionReadCommand.create(w.cfs.metadata(), base.nowInSec(),
        ...
        assertTrue("production bound did not engage during the paging-resume sequence",
                   CursorReads.mergesStoppedByLimit() > stopped);
    }

    // ---------------------------------------------------------------- engagement guards

    /** Byte-identity plus the engagement guard: the production bound must actually have stopped a
     *  merge during the cursor run (limit strictly below the scenario's live-row count), because a
     *  bound that silently fails to engage produces byte-identical output — only the
    ...
    private void assertCursorMatchesWithBoundEngaged(ColumnFamilyStore cfs, Supplier<SinglePartitionReadCommand> cmd)
    {
        ...
        assertTrue("production bound did not engage for this LIMIT scenario (silent non-engagement)",
                   CursorReads.mergesStoppedByLimit() > stopped);
    }
    ...
    private void assertCursorMatchesWithoutBound(ColumnFamilyStore cfs, Supplier<SinglePartitionReadCommand> cmd)
    {
        long stopped = CursorReads.mergesStoppedByLimit();
        assertCursorReadMatchesIterator(cfs, cmd);
        assertEquals("production bound must not engage for this scenario",
