...
/**
 * Differential + scan-metrics-parity scenarios for the combined filter+limit production bound.
 * Before this bound a filtered+limited query got no production bound at all (the limit disengaged
 * because of the filter, the filter's row-level pushdown disengaged because of the limit — two
 ...
 *
 * Every scenario below distinguishes correct composition (a row must survive the filter and the
 * slice-position check before it counts toward the limit) from a plausible-looking but wrong
 * composition (counting rows toward the limit regardless of filter verdict, which would
 * under-produce — the merge stopping early having satisfied the limit with rows the top filter
 * would still have dropped, silently returning fewer real results than the query asked for). The
 * differential harness ({@link #assertCursorReadMatchesIterator}) is the decisive judge: it
 * compares against the iterator path's real {@code RowFilter.filter} then {@code DataLimits.filter}
 * chain, so a wrong composition shows up as a byte divergence, not a hand-derived row count this
 * class would have to get right on its own.
 *
 * <ul>
...
public class LimitedFilterPushdownDifferentialTest extends CursorReadDifferentialTester
{
    ...
    {
        // a STATIC-column filter that fails: the partition-level short-circuit fires before
                                          ...
                                          boolean expectLimitStop, boolean expectFilterDrops)
    {
        ...
        assertTrue("filter pushdown did not engage for a pushable filter under a LIMIT",
                   CursorReads.filterPushdownEngaged() > engagedBefore);
        if (expectLimitStop)
    ...
    protected final class Workload
    {
         ...
         *  the shape the combined bound engages for. */
