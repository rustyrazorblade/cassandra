...
/**
 * Differential and scan-metrics-parity scenarios for CLUSTERING-column filter pushdown. Row
 * groups whose clustering fails a gate-approved expression are dropped inside the cursor merge
 * (never materialized), while every scan metric, threshold, warning text and abort the iterator
 * path's {@code MetricRecording} would have recorded is preserved through the
 * {@code ScanStatsAccumulator}.
 *
 * Scenario families:
 * <ul>
 *   <li><b>Byte identity + engagement guards</b>: clustering filters that keep some rows and drop
 *       others must produce byte-identical results with {@code rowsDroppedByFilter} advancing and
 *       near-zero materialization of the dropped rows.</li>
 ...
 *       {@code TombstoneOverwhelmingException} message and metric deltas), and gcable tombstones
 *       inside dropped rows under both {@code only_purge_repaired_tombstones} settings.</li>
 *   <li><b>LIMIT composition</b>: a LIMITed query with a fully pushable filter engages row-level
 *       pushdown too, cooperating with the production LIMIT bound —
 *       {@link #limitedQueryEngagesClusteringPushdownWhenFilterIsFullyPushable} proves the merge
 *       stops at the limit's satisfaction point without over-scanning into filter-droppable
 *       territory. The full composition matrix lives in
 *       {@code LimitedFilterPushdownDifferentialTest}.</li>
 *   <li><b>Negative control</b>: {@code TEST_SKEW_DROPPED_ROW_ACCOUNTING} silently skips the
 *       dropped rows' tombstone accounting; the parity harness must fail on it.</li>
 *   <li><b>Samplers</b>: the per-partition top-K samplers (not captured by
 *       {@code ScanMetricsCapture}) fold the dropped rows in.</li>
 * </ul>
 */
public class ClusteringFilterPushdownDifferentialTest extends CursorReadDifferentialTester
{
    ...
    {
        // counting order: markers (real and artificial) interleave with dropped rows in stream
        // order, so the crossing element embedded in the exception message must match the
        // iterator path exactly
        Workload w = loadRangeTombstoneWorkload();
    ...
    public void abortThresholdCrossedOnlyByDroppedRowTombstones() throws Throwable
    {
        ...
        try
        {
            // the crossing happens entirely inside dropped rows, so the cursor path must throw at
            // merge time with the same message (count and last-scanned clustering) and the same
            // metric side effects as the iterator path's abort
            Supplier<SinglePartitionReadCommand> cmd =
                w.strictFiltered(f -> f.add(w.col("ck"), Operator.EQ, ByteBufferUtil.bytes(2L)));
            ScanMetricsCapture.Snapshot iterator = assertScanMetricsParity(w.cfs, cmd, true);
            assertTrue("read must have aborted with TombstoneOverwhelmingException", iterator.aborted());
    ...
    public void gcableTombstonesInDroppedRowsArePurgedFromCounts() throws Throwable
    {
        ...
        // on the iterator path, and the cursor accounting must reach the identical counts
    ...
    public void gcableTombstonesInDroppedRowsUnderOnlyPurgeRepaired() throws Throwable
    {
        ...
        // count — proving the accounting matches the full purge predicate, not just gcBefore
    ...
    {
        // a strict regular-column expression drops rows at production; full byte-identity,
        // drop-guard and metrics-parity coverage lives in
        // RegularColumnFilterPushdownDifferentialTest, this keeps the original scenario shape here
        Workload w = loadMixedWorkload();
        ...
        assertTrue("a strict regular-column filter must drop rows at production",
    ...
    {
        Workload w = loadMixedWorkload();
        // this filter (ck < 32) is fully pushable, so the limit bound and clustering pushdown
        // engage together. LIMIT 10 is satisfied well inside the ck < 32 keep-range (around
        // ck 13, since every 4th row is row-tombstoned/dead and doesn't count toward the limit),
        // short of ck = 32 where clustering pushdown would have rows to drop. The merge must stop
        // (mergesStoppedByLimit advances) without reaching any ck >= 32 row: rowsDroppedByFilter
        // must stay untouched, proving the bounded merge does not over-scan past where the top
        // counter would have stopped. The scenario where the limit's satisfaction point falls
        // after filter-dropped rows lives in LimitedFilterPushdownDifferentialTest.
        Consumer<RowFilter> filter = f -> f.add(w.col("ck"), Operator.LT, ByteBufferUtil.bytes(32L));
        ...
        assertTrue("filter pushdown must engage for a pushable filter even under a LIMIT",
                   CursorReads.filterPushdownEngaged() > engagedBefore);
     ...
     *  close, and the abort fires at iterator creation. */
    ...
    protected final class Workload
    {
        ...
        /** The same strict filter under a CQL LIMIT — clustering pushdown engages when the filter
         *  is fully pushable, cooperating with the limit bound; see
         *  {@code limitedQueryEngagesClusteringPushdownWhenFilterIsFullyPushable}. */
     ...
     *
     * @param flushSecond when false the leg-1 writes stay in the memtable (the memtable adapter
     *                    joins the merge and the accounting walk)
     */
