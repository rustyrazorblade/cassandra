...
/**
 * Pins the invariant the cursor validation-support check relies on: a
 * {@link ValidationCompactionController} is always built with {@code compacting = null}, so
 * {@code refreshOverlaps()} leaves the overlapping sstables empty and
 * {@link org.apache.cassandra.db.compaction.CompactionController#shadowSources} always returns
 * null/empty - regardless of the table's {@code provide_overlapping_tombstones} option and of how
 * many other overlapping sstables exist.  If this ever changes, this test must fail loudly.
 */
public class ValidationCompactionControllerTest extends SchemaLoader
{
    ...
    private void assertNeverProvidesShadowSources(String table)
    {
        ...
        // A partition-delete tombstone in its own sstable: the shape that would normally trigger a
        // shadow-sources lookup against overlapping sstables when tombstoneOption != NONE and a
        // real `compacting` set is in play.
        ...
        // compacting - the condition that must NOT produce shadow sources here.
