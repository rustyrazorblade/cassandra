...
public abstract class CommitLogTest
{
    ...
    public void testDontDeleteIfDirty() throws Exception
    {
        ...
        TableStore cfs1 = ks.getColumnFamilyStore(STANDARD1);
        TableStore cfs2 = ks.getColumnFamilyStore(STANDARD2);
    ...
    {
        TableStore cfs = Keyspace.open(keyspace).getColumnFamilyStore(cfName);
    ...
    {
        TableStore cfs = Keyspace.open(KEYSPACE1).getColumnFamilyStore(STANDARD1);
    ...
    {
        Keyspace ks = Keyspace.open(KEYSPACE1);
        TableStore cfs = ks.getColumnFamilyStore(STANDARD1);
        Mutation rm = new RowUpdateBuilder(cfs.metadata(), 0, "k")
                      .clustering("bytes")
    ...
    public void testTruncateWithoutSnapshot() throws ExecutionException, InterruptedException, IOException
    {
        ...
        try
        {
            ...
            for (Keyspace keyspace : Keyspace.system())
                for (TableStore syscfs : keyspace.getColumnFamilyStores())
    ...
    public void testTruncateWithoutSnapshotNonDurable() throws IOException
    {
        ...
        try
        {
            ...
            TableStore cfs = notDurableKs.getColumnFamilyStore("Standard1");
    ...
    public void testOutOfOrderFlushRecovery(BiConsumer<TableStore, Memtable> flushAction, boolean performCompaction)
    ...
    BiConsumer<TableStore, Memtable> flush = (cfs, current) ->
    {
        try
        {
            cfs.forceBlockingFlush();
    ...
    BiConsumer<TableStore, Memtable> recycleSegments = (cfs, current) ->
    {
