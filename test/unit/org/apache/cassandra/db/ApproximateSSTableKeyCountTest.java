...
public class ApproximateSSTableKeyCountTest extends CQLTester
{
    ...
    @Test
    public void testFallsBackToIndexSummaryWhenEstimatorAbsent() throws Exception
    {
        createTable("CREATE TABLE %s (k int PRIMARY KEY, v int)");
        ColumnFamilyStore cfs = getCurrentColumnFamilyStore();
        ...
        // An sstable whose Statistics.db carries no cardinality estimator falls back to the per-sstable
        // index-summary estimate. (CASSANDRA-10676)
        SSTableReader noEstimator = Mockito.spy(sstable);
        Mockito.doReturn(null).when(noEstimator).cardinalityEstimator();

        long fallback = SSTableReader.getApproximateKeyCount(Collections.singletonList(noEstimator));
        assertEquals(sstable.estimatedKeys(), fallback);

        // A repeated read returns the same fallback total.
        assertEquals(fallback, SSTableReader.getApproximateKeyCount(Collections.singletonList(noEstimator)));
    }
