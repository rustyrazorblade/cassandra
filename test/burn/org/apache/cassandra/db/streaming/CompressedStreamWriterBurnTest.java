...
import static org.apache.cassandra.config.CassandraRelevantProperties.TEST_STREAMING_BURN_ITERATIONS;
import static org.apache.cassandra.config.CassandraRelevantProperties.TEST_STREAMING_BURN_MIB;
 ...
 *
 * The test also checksums the received bytes. It compares that with a checksum of the compressed chunks read
 * from the data file. The two match only if the wire bytes equal the on-disk bytes.
 *
 ...
 * is only useful for comparing one run against another. It is not a production number.
 *
...
public class CompressedStreamWriterBurnTest
{
    ...
    private static final int DATA_MIB = TEST_STREAMING_BURN_MIB.getInt();
    private static final int ITERATIONS = TEST_STREAMING_BURN_ITERATIONS.getInt();
    ...
    public void streamCompressedSSTable() throws Exception
    {
        ...
        {
            // first run with a checksum, to confirm the wire bytes equal the on-disk bytes
     ...
     * thread does. The direct buffers the writer takes from the networking pool are not heap, so they do not
