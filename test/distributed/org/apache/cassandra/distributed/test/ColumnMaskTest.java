    ...
    public void testMaskLoading() throws Throwable
    {
        try (Cluster cluster = createClusterWithAuhentication(1))
        {
            ...
            cluster.schemaChange(withKeyspace("CREATE TABLE %s.t (" +
                                              ...
                                              "PRIMARY KEY (a, b))"));
            String insert = withKeyspace("INSERT INTO %s.t(a, b, c, d) VALUES (?, ?, ?, ?)");
            node.executeInternal(insert, "secret1", "secret1", "secret1", "secret1");
            node.executeInternal(insert, "secret2", "secret2", "secret2", "secret2");
